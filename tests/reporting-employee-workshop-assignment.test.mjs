...
const workshopAssignmentPath = '../src/modules/Factory/view/service/workshop/assignment/index.vue';
...
const workshopHistoryPath = '../src/modules/Factory/view/service/workshop/history/index.vue';
...
test('employee commission report exposes KPI cards and chart sections', async () => {
  ...
  assert.match(source, /kpiStatusChartRef/, 'employee report must render a KPI status chart');
  assert.match(
    source,
    /commissionRate/,
...
test('workshop assignment page loads real orders and assigns technicians', async () => {
  const source = await readFile(new URL(workshopAssignmentPath, import.meta.url), {
    encoding: 'utf8',
  });
  ...
  assert.match(source, /technicianWorkloads/, 'assignment page must summarize technician workload');
  assert.match(
    source,
    /unassignedOrders/,
...
test('workshop navigation uses canonical factory workspace routes', async () => {
  const [formSource, detailSource, historySource, factoryMenuSource, workspaceSource] =
    await Promise.all([
...
test('workshop warranty menu opens the warranty request page', async () => {
  const [factoryMenuSource, warrantyRequestsSource, warrantyDetailSource] = await Promise.all([
    readFile(new URL(factoryMenuPath, import.meta.url), {
      encoding: 'utf8',
    }),
