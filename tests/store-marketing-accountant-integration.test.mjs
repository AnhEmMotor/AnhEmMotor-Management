...
test('Store contact and both booking types feed the Marketing inbox contracts', () => {
  ...
  const serviceBookingPage = read('../../AnhEmMotor-Store/app/pages/service-booking.vue');
  const bookingTypes = read('../../AnhEmMotor-Backend/Domain/Constants/Booking/BookingType.cs');
  ...
  const marketingBooking = read('../src/modules/Marketing/view/customer/booking/index.vue');
  const contactApi = read('../src/api/customer/contact.api.ts');
  const contactStore = read('../src/application/store/contact.ts');
  const contactManagement = read('../src/modules/Marketing/view/contact/index.vue');
  ...
  assert.match(contactManagement, /contactStore\.activeItem\.contactId,\s*replyDraft\.value/);
...
test('Employee KPI status is derived from configured targets without fabricated fallbacks', () => {
  ...
  const employeeReport = read('../src/modules/Accountant/view/reporting/employee.vue');
  ...
  assert.doesNotMatch(analyticsRepository, /CommissionPaid\s*=\s*commissionPaid\s*>\s*0/);
...
test('Accountant dashboard period filters reload real KPI data and use reporting surfaces', () => {
  const dashboard = read('../src/modules/Accountant/view/reporting/dashboard.vue');
