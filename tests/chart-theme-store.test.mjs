...
test('chart theme follows the management setting store instead of VueUse document theme state', async () => {
  const source = await readFile(new URL('../src/hooks/core/useChart.ts', import.meta.url), 'utf8')
  ...
  assert.match(source, /storeToRefs/, 'useChart should keep the store-backed isDark value reactive')
})
