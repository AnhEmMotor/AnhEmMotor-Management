...
const listPath = new URL('../src/modules/Admin/view/contract/sales/index.vue', import.meta.url);
const detailPath = new URL(
  '../src/modules/Admin/view/contract/sales/contract-preview.vue',
  import.meta.url
);
const typesPath = new URL('../src/domain/sales/contract.types.ts', import.meta.url);
...
const orderListSource = readFileSync(
  new URL('../src/modules/Order/view/order/contract/index.vue', import.meta.url),
  'utf8'
);
...
test('sales-contract list maps the backend customer and vehicle fields', () => {
  ...
  assert.match(listSource, /deliveryDeadline:\s*contract\.finalPaymentDeadline/);
});

test('delivery deadline shows an exact Vietnam date and a relative calendar-day hint', () => {
  assert.match(listSource, /const VIETNAM_TIME_ZONE\s*=\s*["']Asia\/Ho_Chi_Minh["']/);
  ...
  assert.match(listSource, /getDeliveryDeadlineHint\(scope\.row\.deliveryDeadline\)/);
});

test('the list has one contract entry action and keeps upload inside detail', () => {
  assert.equal((listSource.match(/goToPreview\(scope\.row\.id\)/g) || []).length, 1);
...
test('detail removes addendum actions and all visible statuses are Vietnamese', () => {
  ...
  assert.doesNotMatch(listSource, /Nháp \(Draft\)|Đã ký \(Signed\)|Hoàn tất \(Fulfilled\)/);
...
test('order contract detail keeps the Order sidebar and exposes the submit action', () => {
  ...
  assert.match(orderListSource, /router\.push\(\{\s*name:\s*["']OrderSalesContractPreview["']/);
  assert.match(adminContractRoutesSource, /name:\s*["']SalesContractPreview["']/);
  assert.equal((orderMenuSource.match(/name:\s*["']SalesContractPreview["']/g) ?? []).length, 0);
});
