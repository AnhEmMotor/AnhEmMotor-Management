...
const orderDetailSource = readFileSync(
  new URL('../src/modules/Order/view/order/contract/contract-preview.vue', import.meta.url),
  'utf8'
);
...
test('sales-contract detail keeps app chrome theme-aware and the A4 contract legally styled', () => {
  ...
  assert.match(detailSource, /\.a4-paper[\s\S]*font-family:\s*['"]?Times New Roman/i);
...
test('sales-contract detail uploads image or PDF scans through the API', () => {
  assert.match(detailSource, /accept=["'][^"']*(?:image\/\*|\.png|\.jpe?g)[^"']*["']/i);
  assert.match(detailSource, /accept=["'][^"']*(?:application\/pdf|\.pdf)[^"']*["']/i);
...
test('order staff submit a draft for Admin approval before printing or uploading a scan', () => {
  ...
  assert.match(orderDetailSource, /canPrintContract[\s\S]*"Approved",\s*"Signed",\s*"Fulfilled"/);
  assert.match(
    orderDetailSource,
    /canUploadSignedScan[\s\S]*contractData\.value\.status === "Approved"/
  );
  ...
  assert.match(controllerSource, /HttpPost\("\{id:guid\}\/submit-for-approval"\)/);
