...
for (const page of exportPages) {
  ...
  assert.match(
    source,
    /@\/utils\/report-excel/,
    `${page.name} must use the shared Excel exporter`,
  );
  assert.match(
    source,
  ...
  assert.match(
    source,
    /exportReportWorkbook\s*\(\s*\{/,
    `${page.name} must build a real workbook`,
  );
}
...
const utilitySource = await readFile(
  new URL("src/utils/report-excel.ts", root),
  "utf8",
);
assert.match(
  utilitySource,
...
console.log(
  "Reporting Excel export regression passed for 10 required modules.",
);
