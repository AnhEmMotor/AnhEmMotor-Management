...
const isHomeRoute = (route: RouteLocationMatched): boolean =>
  route.name === "/";
...
const buildFullPath = (childPath: string): string =>
  `/${childPath}`.replace("//", "/");
...
async function handleBreadcrumbClick(
  ...
  try {
    ...
    console.error("Điều hướngThatBai:", error);
  }
}
</script>
