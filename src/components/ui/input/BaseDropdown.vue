...
const internalSelectedLabel = ref('')
...
const selectOption = (value) => {
  ...
  if (selected) {
    internalSelectedLabel.value = selected.text
...
const selectedText = computed(() => {
  ...
  if (internalSelectedLabel.value) return internalSelectedLabel.value
  return props.selectedLabel || props.placeholder
})
...
<template>
  <div class="w-full relative" ref="dropdownRef">
    ...
    >
      <span class="truncate" :class="{ 'text-gray-400': !modelValue && !loading }">{{
        selectedText
      }}</span>
    ...
    >
      ...
      >
        <span
          ...
          class="loader w-4 h-4 border-2 border-gray-200 border-t-red-600 rounded-full animate-spin mr-2"
        ></span>
        <div v-if="pagination.totalPages.value > 0" class="flex items-center space-x-1">
          <button
            ...
            :class="
              pagination.isFirstPage.value || loading
                ? 'bg-gray-100 text-gray-400 border-gray-200 cursor-not-allowed'
                : 'bg-white text-gray-700 border-gray-300 hover:bg-red-50 hover:text-red-600 focus:ring-1 focus:ring-red-500 focus:outline-none'
            "
            title="Trang trước"
          >
            &larr;
          ...
          <div class="flex items-center space-x-1 px-1">
            <template v-for="(page, index) in pageNumbers" :key="index">
              <span v-if="page === '...'" class="px-1 py-1 text-[11px] text-gray-500"> ... </span>
              <button
                v-else
                ...
                :class="[
                  page === pagination.currentPage.value
                    ? 'bg-red-600 text-white border-red-600 shadow-sm focus:outline-none'
          ...
          <button
            ...
            class="inline-flex items-center justify-center px-1.5 py-1 text-[11px] font-medium rounded transition-colors duration-200 border"
            :class="
              pagination.isLastPage.value || loading
                ? 'bg-gray-100 text-gray-400 border-gray-200 cursor-not-allowed'
                : 'bg-white text-gray-700 border-gray-300 hover:bg-red-50 hover:text-red-600 focus:ring-1 focus:ring-red-500 focus:outline-none'
            "
            title="Trang sau"
