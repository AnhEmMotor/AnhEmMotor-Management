<template>
  <div class="min-h-screen flex flex-col">
        ...
        <div class="flex items-center space-x-3">
          <div class="relative hidden sm:block">
            ...
            <input
              ...
              placeholder="Tra theo mã, tên, số điện thoại"
            ...
            <i
              ...
              class="w-4 h-4 text-gray-500 absolute right-3 top-1/2 transform -translate-y-1/2 cursor-pointer hover:text-red-500"
            ></i>
          </div>
    ...
    <main class="flex-1 p-3 max-w-7xl mx-auto w-full flex flex-col lg:flex-row-reverse gap-5">
        ...
        <div class="mb-5">
          ...
          <div class="space-y-2">
            <div class="flex space-x-2">
              <input
                type="text"
                :value="filters.totalPurchaseMin"
                @input="updateFormattedNumberFilter('totalPurchaseMin', $event)"
                placeholder="Từ"
                class="w-1/2 text-sm filter-input focus:ring-red-500 focus:border-red-500"
              />
              <input
                type="text"
                :value="filters.totalPurchaseMax"
                @input="updateFormattedNumberFilter('totalPurchaseMax', $event)"
                placeholder="Đến"
                class="w-1/2 text-sm filter-input focus:ring-red-500 focus:border-red-500"
              />
            </div>
            ...
            <label class="block">
              ...
              >
                <input
                  ...
                  value="custom"
                  v-model="filters.timeFilter"
                  class="form-radio text-red-600 h-4 w-4 border-gray-300 focus:ring-red-500 mr-3"
                />
      ...
      <section class="w-full lg:w-3/4">
        <div class="bg-white p-4 rounded-xl shadow-md border border-gray-200">
          <div
            class="grid grid-cols-6 gap-4 text-xs font-semibold text-gray-600 border-b pb-1 mb-2"
          >
          ...
          <div v-if="filteredSuppliers.length > 0">
            <div
              ...
              :key="supplier.id"
              class="supplier-row-wrapper border border-gray-200 rounded-lg overflow-hidden mb-2"
            >
              <div
                class="grid grid-cols-6 gap-4 items-center py-2 px-3 text-sm cursor-pointer transition-colors hover:bg-gray-50"
              ...
              >
                <div class="col-span-2 flex items-center space-x-3">
                  <input
                    ...
                    @change="toggleSupplierDetail(supplier.id)"
                    @click.stop
                    class="form-checkbox text-red-600 rounded w-4 h-4 focus:ring-red-500"
                  />
                  <i
                    :data-lucide="
                      selectedSupplierId === supplier.id ? 'chevron-down' : 'chevron-right'
                    "
                  ...
                  <span
                    ...
                    :class="{ 'text-primary': selectedSupplierId === supplier.id }"
                    >{{ supplier.name }}</span
                  >
                </div>
                ...
                <div class="text-right">
                  <span
                    ...
                    :class="getSupplierStatusClass(supplier.status)"
                  >
              ...
              <div
                v-if="selectedSupplierId === supplier.id"
              ...
              >
                <div class="flex border-b border-gray-200 mb-3 space-x-5">
                  <button
                    ...
                    :class="
                      activeTab === 'info'
                        ? 'tab-active'
                        : 'text-gray-600 hover:border-b-2 hover:border-gray-300 transition-colors'
                    "
                ...
                <div v-show="activeTab === 'info'">
                  <h3 class="text-lg font-bold text-gray-800 mb-3">
                    {{ supplier.name }}
                    <span class="text-sm font-normal text-gray-500 ml-2">{{ supplier.id }}</span>
                  </h3>
                  <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-3 mb-4">
                    <div class="text-sm">
                      ...
                      ><span class="font-medium text-gray-700">{{ supplier.creator }}</span>
                    </div>
                    <div class="text-sm">
                      <span class="text-gray-500 block">Ngày tạo:</span
                      ><span class="font-medium text-gray-700">{{
                        formatDate(supplier.creationDate)
                      }}</span>
                    </div>
                  </div>
                  ...
                  <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-3">
                    ...
                    <div class="text-sm col-span-1 md:col-span-2 lg:col-span-3">
                      ...
                      ><span class="font-medium text-gray-800">{{
                        [supplier.address, supplier.ward, supplier.cityDistrict]
                          .filter(Boolean)
                          .join(', ') || 'Chưa có'
                      }}</span>
                    </div>
                  ...
                  <div class="flex justify-end space-x-3 mt-4 pt-3 border-t border-gray-100">
                    ...
                    <button
                      ...
                      @click="openAddEditModal(supplier)"
                    >
                    ...
                    <button
                      ...
                      @click="toggleActivation(supplier.id)"
                    >
                ...
                <div v-show="activeTab === 'history'">
                  <div
                    v-if="
                      ...
                      supplier.transactions.filter((t) => t.status === 'Đã nhập hàng').length > 0
                    "
                    class="p-4 border border-gray-200 rounded-lg bg-white"
                  >
                    <h4 class="font-semibold text-gray-800 mb-3">Lịch sử giao dịch</h4>
                    <div
                      class="grid grid-cols-5 gap-4 text-xs font-semibold text-gray-600 border-b pb-2 mb-2"
                    >
                      <div>Mã phiếu</div>
                      <div>Thời gian</div>
                      <div>Người tạo</div>
                    ...
                    <div>
                      <div
                        v-for="t in supplier.transactions.filter(
                          (tx) => tx.status === 'Đã nhập hàng',
                        )"
                        :key="t.code"
    ...
    <div
      ...
      class="modal fixed inset-0 bg-gray-900 bg-opacity-50 z-50 flex items-center justify-center"
    >
      <div class="bg-white p-6 rounded-xl shadow-2xl w-full max-w-3xl">
        <div class="flex justify-between items-center border-b pb-3 mb-4">
          <h3 class="text-xl font-bold text-gray-800">
            {{ editableSupplier.id ? 'Chỉnh sửa Nhà cung cấp' : 'Tạo nhà cung cấp' }}
          </h3>
          <button @click="isAddEditModalVisible = false" class="text-gray-400 hover:text-gray-600">
        ...
        <form @submit.prevent="handleSaveSupplier">
          <div class="space-y-4 max-h-[70vh] overflow-y-auto pr-2">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                ><input
                  type="text"
                  :value="editableSupplier.id || 'Tự động'"
                  disabled
            ...
            <div class="border border-gray-200 rounded-lg">
              <button
                ...
                @click="(e) => e.currentTarget.nextElementSibling.classList.toggle('hidden')"
              >
                Địa chỉ
              ...
              <div class="p-4 border-t border-gray-200 space-y-4 hidden">
                ...
                <div class="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    ><input
                      type="text"
                      v-model="editableSupplier.cityDistrict"
                      class="mt-1 w-full filter-input focus:border-red-500"
          ...
          <div class="mt-6 flex justify-end space-x-3 pt-4 border-t border-gray-200">
            <button
              type="button"
              @click="isAddEditModalVisible = false"
    ...
    >
      <div class="bg-white p-6 rounded-xl shadow-2xl w-full max-w-md">
        <div class="flex justify-between items-center border-b pb-3 mb-4">
          ...
          <button @click="isAddGroupModalVisible = false" class="text-gray-400 hover:text-gray-600">
            <i data-lucide="x" class="w-6 h-6"></i>
          </button>
        </div>
        <form @submit.prevent="handleAddGroup">
          <div class="space-y-4">
            ...
            <div>
              <label for="group-notes" class="block text-sm font-medium text-gray-700"
                >Ghi chú</label
              ><textarea
      ...
      <div class="bg-white p-6 rounded-xl shadow-2xl w-full max-w-sm">
        <div class="flex justify-between items-start border-b pb-3 mb-4">
          <h3 class="text-xl font-bold text-red-600 flex items-center">
            <i data-lucide="alert-triangle" class="w-6 h-6 mr-2 text-red-500"></i>Xác nhận xóa
          </h3>
          <button @click="isDeleteModalVisible = false" class="text-gray-400 hover:text-gray-600">
            <i data-lucide="x" class="w-6 h-6"></i>
          </button>
        </div>
        <p class="text-gray-700 mb-6">
          Bạn có chắc chắn muốn xóa nhà cung cấp **<span class="font-semibold">{{
            supplierToDelete.name
          }}</span
          >** (ID: <span class="font-mono text-xs">{{ supplierToDelete.id }}</span
    ...
    <div
      ...
      :class="{ 'bg-red-600': message.type === 'success', 'bg-red-800': message.type === 'error' }"
    >
      {{ message.text }}
    </div>
  </div>
...
function handleSaveSupplier() {
  ...
  if (editableSupplier.value.id) {
    ...
    if (index !== -1) {
      suppliers.value[index] = editableSupplier.value
    }
...
watch(
  [isAddEditModalVisible, isAddGroupModalVisible, isDeleteModalVisible, selectedSupplierId],
  () => {
    nextTick(() => {
      if (window.lucide) window.lucide.createIcons()
    })
