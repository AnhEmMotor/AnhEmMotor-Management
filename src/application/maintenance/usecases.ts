...
import { RepairOrderApi, type RepairOrderDetail } from '@/infrastructure/api/repair-order'
...
export interface UpdateOdoAndGenerateWarningsUseCase {
  execute(payload: { ticketId: number; currentOdo: number }): Promise<MaintenanceTicket>
}

export interface ApplyMaintenancePackageUseCase {
  execute(payload: { ticketId: number; packageId: string }): Promise<MaintenanceTicket>
}

export interface IssuePartsUseCase {
  execute(payload: { ticketId: number; parts: MaintenancePartsLine[] }): Promise<MaintenanceTicket>
}

export interface ChangeWorkflowStatusUseCase {
  execute(payload: { ticketId: number } & MaintenanceWorkflowPayload): Promise<void>
}
...
function computeWarningsByOdo(
  ...
  const res: Array<{ level: 'warning' | 'danger'; targetKm: number; message: string }> = []
  ...
  return [
    {
      ...
      discount: 0,
      checklist: [
        { id: 'c_brake', name: 'Kiểm tra hệ thống phanh trước/sau', done: false },
    ...
    },
    {
      ...
      checklist: [
        { id: 'c_brake', name: 'Kiểm tra hệ thống phanh trước/sau', done: false },
        { id: 'c_tire', name: 'Kiểm tra áp suất lốp', done: false },
        { id: 'c_air_filter', name: 'Thay thế lọc gió (Khách đã chốt thay [✓])', done: false },
      ],
    },
    {
      id: 'pkg_15000',
      ...
      checklist: [
        ...
        { id: 'c_engine', name: 'Vệ sinh nồi/đường dầu (tùy mốc)', done: false },
        { id: 'c_brake', name: 'Kiểm tra hệ thống phanh trước/sau', done: false },
      ],
    },
  ]
...
function applyPackage(ticket: MaintenanceTicket, pkg: MaintenancePackage): MaintenanceTicket {
  return {
    ...ticket,
    selectedPackage: pkg,
...
function computeTotalPartsCost(parts: MaintenancePartsLine[]) {
  return parts.reduce((sum, p) => sum + (p.total || 0), 0)
}
...
export class RealUpdateOdoAndGenerateWarningsUseCase implements UpdateOdoAndGenerateWarningsUseCase {
  async execute(payload: { ticketId: number; currentOdo: number }): Promise<MaintenanceTicket> {
    ...
    if (payload.currentOdo < (ticket.lastOdoBefore ?? 0)) {
      throw new Error('ODO không được nhỏ hơn ODO của lần bảo dưỡng liền trước.')
    }
...
export class RealApplyMaintenancePackageUseCase implements ApplyMaintenancePackageUseCase {
  async execute(payload: { ticketId: number; packageId: string }): Promise<MaintenanceTicket> {
    const ticket = await new RealGetMaintenanceTicketUseCase().execute({
      ticketId: payload.ticketId,
    })
    const packages = getPackageTemplates()
...
export class RealIssuePartsUseCase implements IssuePartsUseCase {
  async execute(payload: {
    ...
    return new RealGetMaintenanceTicketUseCase().execute({ ticketId: payload.ticketId })
  }
}

export class RealChangeWorkflowStatusUseCase implements ChangeWorkflowStatusUseCase {
  async execute(payload: { ticketId: number } & MaintenanceWorkflowPayload): Promise<void> {
