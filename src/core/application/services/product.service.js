...
const productService = {
  ...
  async getOptions() {
    const { data } = await axiosInstance.get('/api/v1/product/all-options')
    return data
  },

  async createOptionValue(payload) {
    const { data } = await axiosInstance.post('/api/v1/product/option-values', payload)
    return data
  },

  async updateOptionValue(id, payload) {
    const { data } = await axiosInstance.put(`/api/v1/product/option-values/${id}`, payload)
    return data
  },

  async deleteOptionValue(id) {
    const { data } = await axiosInstance.delete(`/api/v1/product/option-values/${id}`)
    return data
  }
}
