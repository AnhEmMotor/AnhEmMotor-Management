...
export const useCategoryStore = defineStore('category', () => {
  ...
  // Dùng cho bảng quản lý - chỉ lấy CategoryGroup="Product"
  const fetchCategoriesForManager = async (query) => {
    isLoading.value = true
    error.value = null
    try {
      const params = categoryMapper.toParams(query)
      const data = await categoryService.fetchCategoriesForManager(params)
      return {
        data: categoryMapper.toList(data),
        pagination: categoryMapper.toPagination(data),
      }
