...
export const ContactApi = {
  getPaginated(params: { contactType?: string; status?: string; page?: number; pageSize?: number }) {
    return request.get<PaginatedContactResponse>({ url: "/api/v1/Contacts/paginated", params })
  },
  updateStatus(id: number, contactType: string, data: Contact.UpdateStatusRequest) {
    return request.patch<void>({ url: `/api/v1/Contacts/${id}/status?contactType=${contactType}`, data })
  },
  ...
  createSupportRequest(data: Contact.CreateSupportRequestPayload) {
    return request.post<number>({ url: "/api/v1/Contacts/support-request", data })
  },
  ...
  createJobApplication(data: Contact.CreateJobApplicationPayload) {
    return request.post<number>({ url: "/api/v1/Contacts/job-application", data })
  },
}
