...
export type ReportExcelCell = string | number | boolean | Date | null | undefined;
...
function sanitizeRows(rows: ReportExcelRow[]) {
  return (rows.length > 0 ? rows : [EMPTY_ROW]).map((row) =>
    Object.fromEntries(Object.entries(row).map(([key, value]) => [key, sanitizeCellValue(value)]))
  );
}
...
export function exportReportWorkbook({ fileName, sheets }: ExportReportWorkbookOptions) {
