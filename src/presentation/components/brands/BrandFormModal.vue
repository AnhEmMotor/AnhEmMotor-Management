...
const editableBrand = ref({ ...props.brand })
const formErrors = ref({ name: '' })

watch(
  () => props.brand,
  (newVal) => {
    editableBrand.value = { ...newVal }
  },
  { deep: true },
)
    ...
    <template #body>
      <div class="space-y-4 p-1">
        ...
        <div>
          ...
          <Input v-model="editableBrand.origin" placeholder="Ví dụ: Nhật Bản, Ý, Việt Nam..." />
        </div>
        <div>
          <BaseImage
            label="Logo Thương hiệu"
            v-model="editableBrand.logoUrl"
          />
        </div>
        <div>
