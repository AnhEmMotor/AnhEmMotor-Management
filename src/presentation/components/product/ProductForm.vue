...
const { data: categoriesData, isLoading: isCategoriesLoading } = useQuery({
  ...
  queryFn: () => categoryStore.getAllCategories({ PageSize: 1000 }),
  staleTime: 5 * 60 * 1000,
})
...
const { data: technologiesData, isLoading: isTechnologiesLoading } = useQuery({
  ...
  queryFn: () => productService.getTechnologiesList(),
  staleTime: 5 * 60 * 1000,
})
...
const techCategoryOptions = computed(() => {
  const categories = techCategoriesData.value || []
...
const techCategoryMap = computed(() => {
  const categories = Array.isArray(techCategoriesData.value) ? techCategoriesData.value : []
  return new Map(categories.map((c) => [c.id, c.name]))
})

const groupedTechnologies = computed(() => {
  const techs = Array.isArray(technologiesData.value) ? technologiesData.value : []
  const groups = {}
...
const createTechMutation = useMutation({
  ...
  onMutate: async (newTechData) => {
    ...
    const optimisticTech = {
      ...
      category: {
        name:
          techCategoryOptions.value.find((c) => c.value === newTechData.categoryId)?.text ||
          'Đang tạo...',
      },
      isOptimistic: true,
    }
...
const tabs = computed(() => {
  const t = [{ id: 'common', label: 'Thông tin chung', icon: 'fa-info-circle' }]
...
const categoryOptions = computed(() => {
  ...
  return flattened.map((c) => ({
    value: c.id,
    text: '\u00A0\u00A0'.repeat(c.level) + (c.level > 0 ? '└ ' : '') + c.name,
  }))
})
...
watch(
  ...
  (newVal) => {
    ...
    if (
      newVal?.id === localProduct.value?.id &&
    ...
    // --- TECHNICAL SPECS (VEHICLE) ---
    copy.engine_type = copy.engine_type || ''
    copy.max_power = copy.max_power || ''
    copy.fuel_consumption = copy.fuel_consumption || ''
    copy.transmission_type = copy.transmission_type || ''
    copy.starter_system = copy.starter_system || ''
    copy.max_torque = copy.max_torque || ''
    copy.bore_stroke = copy.bore_stroke || ''
    copy.compression_ratio = copy.compression_ratio || ''
    copy.fuel_system = copy.fuel_system || ''
    copy.frame_type = copy.frame_type || ''
    copy.front_tire_size = copy.front_tire_size || ''
    copy.rear_tire_size = copy.rear_tire_size || ''
    copy.front_brake = copy.front_brake || ''
    copy.rear_brake = copy.rear_brake || ''
    copy.battery_type = copy.battery_type || ''
    copy.lighting_system = copy.lighting_system || ''
    copy.dashboard_type = copy.dashboard_type || ''
    // ---------------------------------
    ...
    if (!copy.variants || copy.variants.length === 0) {
      ...
      copy.variants.forEach((v) => {
        ...
        if (!v.url_slug)
          v.url_slug = generateVariantSlug(
            copy.name,
            v.optionValues,
            v.version_name,
            v.color_name,
...
const updateAllVariantSlugs = () => {
  if (localProduct.value.variants) {
    localProduct.value.variants.forEach((v) => {
      if (!props.isEditMode || !v.id || !v.url_slug) {
        v.url_slug = generateVariantSlug(
...
const generateSKU = (name, version, color) => {
  ...
  const parts = [sanitize(name), sanitize(version), sanitize(color)].filter((p) => !!p)
  return parts.join('-')
}
    ...
    localProduct.value.variants.forEach((variant) => {
      if (!variant.is_sku_manual) {
        const colorNames = variant.colors
          ...
          .filter((n) => !!n)
          .join('-')
        const newSKU = generateSKU(localProduct.value.name, variant.version_name, colorNames)
        if (newSKU) {
          variant.sku = newSKU
        }
      }
...
const addVariant = () => {
  ...
  newVariant.url_slug = generateVariantSlug(
    localProduct.value.name,
    newVariant.optionValues,
    newVariant.version_name,
...
const getVariantErrors = (index) => {
  ...
  return {
    ...
    sku: vErrors.sku || '',
    price: vErrors.price || '',
    url: vErrors.url_slug || vErrors.url || vErrors.UrlSlug || '',
  }
}
...
const toggleTechnology = (tech) => {
  ...
  const existingIndex = localProduct.value.highlights_list.findIndex(
    (h) => h.technology_id === tech.id,
  )
  ...
  if (existingIndex >= 0) {
    ...
    localProduct.value.highlights_list.push({
      ...
      custom_description: tech.defaultDescription || '', // Auto-fill from image data
      ...
      _categoryName: tech.categoryName || techCategoryMap.value.get(tech.categoryId) || 'Khác',
    })
  }
...
const isTechnologySelected = (techId) => {
  ...
  return localProduct.value.highlights_list.some(
    (h) => Number(h.technology_id) === Number(techId),
  )
}
...
const removeCompatibleVehicle = (id) => {
  localProduct.value.compatible_vehicle_model_ids =
    localProduct.value.compatible_vehicle_model_ids.filter((vId) => vId !== id)
}
</script>
...
<template>
  ...
  <!-- Tab Navigation -->
  <div
    class="flex border-b border-gray-200 sticky top-0 bg-white z-20 px-6 overflow-x-auto overflow-y-hidden no-scrollbar shadow-sm w-full"
  >
    <button
      v-for="tab in tabs"
      :key="tab.id"
            ...
            <Input
              ...
              :placeholder="
                !isTypeSelected
                  ? 'Nhập tên sản phẩm...'
                  : isVehicle
                    ? 'Ví dụ: Honda Winner X'
                    : 'Ví dụ: Nhông sên dĩa DID'
              "
              :error="errors?.name"
              required
            />
      ...
      <div v-if="isPart" v-show="activeTab === 'compatibility'">
        <div class="space-y-4">
          <p class="text-sm text-gray-600 mb-6 italic">
            QUAN TRỌNG: Chọn các dòng xe mà món phụ tùng này có thể lắp vừa. Hệ thống sẽ dùng dữ
            liệu này để gợi ý sản phẩm liên quan.
          </p>
          ...
          <div class="space-y-4">
            <div class="relative">
              ...
              >
                ...
                >
                  <span>{{ v.name }}</span>
                  <span
                    v-if="localProduct.compatible_vehicle_model_ids?.includes(v.id)"
            ...
            <div class="flex flex-wrap gap-2">
              ...
              <div
                v-if="!localProduct.compatible_vehicle_model_ids?.length"
      ...
      <div v-show="activeTab === 'specs'">
        ...
        <div v-if="isVehicle" class="space-y-6">
          <div class="space-y-8">
            ...
            <div class="space-y-4">
              <h4
                class="text-[11px] font-black text-primary-600 uppercase tracking-widest border-l-4 border-primary-500 pl-3 py-1 bg-primary-50/30"
              >
                1. Kích thước & Trọng lượng
              </h4>
              <div class="grid grid-cols-1 md:grid-cols-3 gap-x-4 gap-y-3">
                <Input
                  ...
                  placeholder="ví dụ: 1.925 x 705 x 1.120 mm"
                />
                <Input
            ...
            <div class="space-y-4">
              ...
              <div class="grid grid-cols-1 md:grid-cols-3 gap-x-4 gap-y-3">
                <Input
                  ...
                  placeholder="ví dụ: 4 kỳ, 1 xi-lanh, SOHC"
                  class="md:col-span-2"
                />
                <Input
                  ...
                  placeholder="ví dụ: 149.3"
                />
                <Input
                  label="Đường kính x Hành trình piston (mm)"
                  v-model="localProduct.bore_stroke"
                  placeholder="ví dụ: 57,3 x 57,9"
                />
                <Input
                  label="Tỷ số nén"
                  v-model="localProduct.compression_ratio"
                  placeholder="ví dụ: 10,6:1"
                />
                <Input
                  label="Công suất tối đa"
                  v-model="localProduct.max_power"
                  placeholder="ví dụ: 11,3 kW / 8.500 vòng/phút"
                />
                <Input
                  label="Moment xoắn cực đại"
                  v-model="localProduct.max_torque"
                  placeholder="ví dụ: 13,8 Nm / 7.000 vòng/phút"
                />
                <Input
                  label="Hệ thống cung cấp nhiên liệu"
                  v-model="localProduct.fuel_system"
                  placeholder="ví dụ: Phun xăng điện tử (FI)"
                />
                <Input
                  label="Loại truyền động (Hộp số)"
                  v-model="localProduct.transmission_type"
                  placeholder="ví dụ: Côn tay 6 số"
                />
                <Input
                  label="Hệ thống khởi động"
                  v-model="localProduct.starter_system"
                  placeholder="ví dụ: Điện / Đạp chân"
                />
                <Input
                  label="Mức tiêu thụ nhiên liệu"
                  v-model="localProduct.fuel_consumption"
                  placeholder="ví dụ: 2,1 lít / 100km"
                />
                <Input
            ...
            <div class="space-y-4">
              ...
              <div class="grid grid-cols-1 md:grid-cols-3 gap-x-4 gap-y-3">
                <Input
                  label="Loại khung xe"
                  v-model="localProduct.frame_type"
                  placeholder="ví dụ: Khung ống thép"
                />
                <Input
                  label="Phuộc trước"
                ...
                <Input
                  ...
                  placeholder="ví dụ: Lò xo trụ đôi"
                />
                <Input
                  label="Kích cỡ lốp trước"
                  v-model="localProduct.front_tire_size"
                  placeholder="ví dụ: 90/80-17"
                />
                <Input
                  label="Kích cỡ lốp sau"
                  v-model="localProduct.rear_tire_size"
                  placeholder="ví dụ: 120/70-17"
                />
                <Input
                  label="Phanh trước"
                  v-model="localProduct.front_brake"
                  placeholder="ví dụ: Phanh đĩa (ABS)"
                />
                <Input
                  label="Phanh sau"
                  v-model="localProduct.rear_brake"
                  placeholder="ví dụ: Phanh đĩa"
                />
              </div>
            </div>
            ...
            <div class="space-y-4">
              ...
              <div class="grid grid-cols-1 md:grid-cols-3 gap-x-4 gap-y-3">
                <Input
                  label="Loại ắc quy"
                  v-model="localProduct.battery_type"
                  placeholder="ví dụ: 12V - 5Ah"
                />
                <Input
                  label="Hệ thống chiếu sáng"
                  v-model="localProduct.lighting_system"
                  placeholder="ví dụ: LED"
                />
                <Input
                  label="Đồng hồ hiển thị"
                  v-model="localProduct.dashboard_type"
                  placeholder="ví dụ: LCD"
                />
              </div>
            </div>
          </div>
        ...
        <div v-else class="space-y-6">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <Input
              ...
              placeholder="ví dụ: Nhôm, Thép, Nhựa ABS..."
            />
            <Input
              label="Xuất xứ"
              v-model="localProduct.origin"
              placeholder="ví dụ: Thái Lan, Nhật Bản, Việt Nam..."
            />
            <Input
              label="Thời gian bảo hành"
              v-model="localProduct.warranty_period"
      ...
      <div v-show="activeTab === 'tech'">
        <div class="space-y-6">
          <div v-if="isVehicle" class="space-y-6">
            ...
            <div
              v-else-if="
                technologiesData && Array.isArray(technologiesData) && technologiesData.length > 0
              "
              class="space-y-8"
            >
                  ...
                  <div
                    ...
                    :class="
                      isTechnologySelected(tech.id)
                        ? 'border-red-600 bg-red-50/30 shadow-sm ring-1 ring-red-600/20'
                        : tech.isOptimistic 
                          ? 'border-gray-100 bg-gray-50 opacity-60 cursor-not-allowed'
                          : 'border-gray-100 hover:border-red-200 hover:bg-gray-50'
                    "
                    @click="!tech.isOptimistic && toggleTechnology(tech)"
                  >
                    <div class="flex items-center gap-3">
                      ...
                      >
                        <i
                          v-if="isTechnologySelected(tech.id)"
                      ...
                      <span
                        ...
                        :class="
                          isTechnologySelected(tech.id) ? 'text-red-900' : 'text-gray-700'
                        "
                        >{{ tech.name }}</span
                      >
                    </div>

                    <div v-if="isTechnologySelected(tech.id)" class="text-red-600">
                      <i class="fas fa-circle-check text-sm"></i>
                    </div>
                    <div v-else-if="tech.isOptimistic" class="text-gray-400">
            ...
            <div
              v-if="localProduct.highlights_list && localProduct.highlights_list.length > 0"
              class="mt-8 space-y-6"
            >
              <h4 class="font-medium text-gray-900 border-b pb-2 italic text-sm">
                Tùy chỉnh nội dung công nghệ cho xe này (Hiện trên Web)
              </h4>

              <div
                v-for="(hl, hlIdx) in localProduct.highlights_list"
                :key="hlIdx"
                class="p-4 border rounded-xl bg-gray-50/30 relative group"
              >
                <div class="flex items-center gap-2 mb-4">
                  <span
                    class="bg-primary-100 text-primary-800 text-[10px] px-2 py-0.5 rounded font-bold uppercase tracking-wider"
                    >{{ hl._categoryName }}</span
                  >
                  <span class="font-bold text-gray-800">{{
                    (Array.isArray(technologiesData) ? technologiesData : [])?.find(
                      (t) => t.id === hl.technology_id,
                    )?.name || 'Unknown'
                  }}</span>
                </div>
        ...
        <div v-else class="space-y-6">
            <p class="text-sm text-gray-500 italic">
              Thiết lập các tiêu chuẩn an toàn và chứng nhận chất lượng cho phụ tùng/phụ kiện.
            </p>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div
                class="p-4 border rounded-lg hover:border-primary-300 transition-colors bg-gray-50/50"
              >
                <div class="flex items-center gap-3">
                  <input
                    type="checkbox"
                    v-model="localProduct.std_dot"
                    class="w-4 h-4 text-primary-600 rounded"
                  />
                  <div>
                    <div class="font-bold">Tiêu chuẩn DOT</div>
                    <div class="text-xs text-gray-500">
                      Tiêu chuẩn an toàn của Bộ Giao thông vận tải Hoa Kỳ.
                    </div>
                  </div>
                </div>
              </div>
              ...
              >
                <div class="flex items-center gap-3">
                  <input
                    type="checkbox"
                    v-model="localProduct.std_ece"
                    class="w-4 h-4 text-primary-600 rounded"
                  />
                  <div>
                    <div class="font-bold">Tiêu chuẩn ECE</div>
                    <div class="text-xs text-gray-500">
                      Tiêu chuẩn an toàn của Ủy ban Kinh tế Châu Âu.
                    </div>
                  </div>
                </div>
              </div>
              ...
              >
                <div class="flex items-center gap-3">
                  <input
                    type="checkbox"
                    v-model="localProduct.std_snell"
                    class="w-4 h-4 text-primary-600 rounded"
                  />
                  <div>
                    <div class="font-bold">Tiêu chuẩn SNELL</div>
                    <div class="text-xs text-gray-500">
                      Tiêu chuẩn an toàn cao cấp dành cho nón bảo hiểm đua.
                    </div>
                  </div>
                </div>
              </div>
            ...
            <Input
              ...
              v-model="localProduct.other_standards"
              placeholder="ví dụ: TCVN, AS/NZS..."
            />
          </div>
        </div>
      </div>
          ...
          <div class="space-y-6 mt-4">
            ...
            >
              ...
              <div class="flex items-center gap-3 mb-6">
                <div
                  class="w-8 h-8 rounded-full bg-primary-600 text-white flex items-center justify-center font-bold text-sm shadow-sm"
                >
                  {{ index + 1 }}
                </div>
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
                <Input
                  ...
                  :placeholder="
                    isVehicle ? 'ví dụ: Tiêu chuẩn, Đặc biệt...' : 'ví dụ: Size L, Bộ 3 món...'
                  "
              ...
              <div v-if="isVehicle" class="mb-8">
                <div class="flex items-center gap-2 mb-4">
                  ...
                  <label class="text-sm font-bold text-gray-700"
                    >Ghi đè thông số kỹ thuật (Spec Overriding)</label
                  >
                  <button
                    type="button"
                ...
                <div
                  v-if="variant.showSpecs"
                  class="grid grid-cols-1 md:grid-cols-3 gap-4 p-5 border rounded-2xl bg-blue-50/10 border-blue-100 shadow-inner"
                >
                  <Input
                  ...
                  <Input
                    label="Trục bánh xe (mm)"
                    type="number"
                    v-model.number="variant.wheelbase"
                  />
                  <Input
                    label="Độ cao yên (mm)"
                    type="number"
                    v-model.number="variant.seat_height"
                  />
                  <Input
                    label="Khoảng sáng gầm (mm)"
                    type="number"
                    v-model.number="variant.ground_clearance"
                  />
                  <Input
                    label="Dung tích xăng (lít)"
                    type="number"
                    step="0.1"
                    v-model.number="variant.fuel_capacity"
                  />
                  <Input label="Kích cỡ lốp" v-model="variant.tire_size" placeholder="Dùng gốc" />
                  <div
                    class="md:col-span-3 text-[11px] text-blue-600 italic bg-blue-50 p-2 rounded border border-blue-100 flex items-center gap-2"
                  >
              ...
              <div class="mb-8">
                <div class="flex items-center gap-2 mb-4">
                  ...
                  <label class="text-sm font-bold text-gray-700">{{
                    isVehicle ? 'Bộ sưu tập Hình ảnh theo Màu' : 'Màu sắc & Hình ảnh'
                  }}</label>
                </div>

                <div class="space-y-6 p-6 border rounded-2xl bg-white shadow-sm border-gray-100">
                  <div
                    v-for="(color, cIndex) in variant.colors"
                    :key="cIndex"
                    class="p-5 border border-gray-50 rounded-2xl bg-gray-50/30"
                  >
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                          ...
                          <div class="flex flex-col">
                            <label class="text-sm font-medium text-gray-700 mb-1"
                              >Mã màu hiển thị</label
                            >
                            <div
                              class="flex gap-3 items-center h-11 px-3 border rounded-xl bg-white shadow-inner"
                            >
                              <input
                                type="color"
                                v-model="color.code"
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                <Input
                  label="URL Slug (SEO) *"
                  v-model="variant.url_slug"
                  placeholder="ten-san-pham-bien-the"
                  :error="getVariantErrors(index).url"
                  @blur="variant.url_slug = slugify(variant.url_slug)"
                />
                <Input
                  v-if="!isVehicle"
              ...
              <div class="space-y-4">
                <div class="flex items-center gap-2">
                  ...
                  <label class="text-sm font-bold text-gray-700"
                    >Bộ sưu tập ảnh chi tiết (Gallery)</label
                  >
                </div>
                <GroupImage
  ...
  >
    <template #body>
      <div class="space-y-5">
        <p class="text-sm text-gray-500 italic">
          Lưu ý: Công nghệ bạn thêm ở đây sẽ được lưu vào danh mục chung của hệ thống để sử dụng cho
          các xe khác sau này.
        </p>
...
/* Normalize label styles to match Dropdown and Textarea */
:deep(label) {
