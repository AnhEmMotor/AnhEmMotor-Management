<template>
  <div class="w-full">
    <label v-if="label" :for="inputId" class="block text-sm font-semibold text-gray-700 mb-1">
      {{ label }}
    </label>
    ...
    <input
      ...
      :class="[
        'w-full border rounded-md focus:outline-none transition-all duration-300',
        size === 'sm' ? 'p-1 text-[13px]' : 'p-1.5 text-sm',
        inputClasses,
        inputClass,
      ]"
      v-bind="$attrs"
