...
const handleGoogleLogin = (response) => {
  ;(async () => {
    try {
      ...
      toast.error(err.response?.data?.message || err.response?.data?.errors?.[0]?.message || 'Đăng nhập Google thất bại')
    } finally {
      ...
      if (response.authResponse) {
        ;(async () => {
          try {
            ...
            toast.error(err.response?.data?.message || err.response?.data?.errors?.[0]?.message || 'Đăng nhập Facebook thất bại')
          } finally {
            isLoading.value = false
            emit('loading', false)
          }
