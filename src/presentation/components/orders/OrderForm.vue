...
const estimatedCommission = computed(() => {
  return localData.value.products.reduce((sum, p) => {
    // Giả định: Tên sản phẩm có chữ 'xe' hoặc giá > 10tr thì coi là Xe máy
    const productName = p.name || ''
    const isVehicle = productName.toLowerCase().includes('xe') || p.unitPrice >= 10000000
    if (isVehicle) {
      return sum + (p.quantity * 500000) // 500k mỗi xe
    } else {
      return sum + (p.total * 0.03) // 3% cho phụ tùng/phụ kiện
    }
  }, 0)
})
      ...
      <div class="flex items-center justify-between w-full">
        <div class="flex flex-col gap-1 pl-6 pb-4">
          <div v-if="localData.shippingFee > 0" class="text-sm text-gray-500 font-medium">
            Phí giao hàng: {{ localData.shippingFee.toLocaleString('vi-VN') }} VNĐ
          </div>
          <div class="text-xl font-bold text-primary-600">Tổng: {{ totalAmount.toLocaleString('vi-VN') }} VNĐ</div>
        ...
        <div class="flex gap-2 pr-6 pb-4">
