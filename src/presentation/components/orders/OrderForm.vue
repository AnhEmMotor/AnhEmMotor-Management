...
import { formatCurrency, parseCurrency, formatVNDWithUnit } from '@/utils/currency'
...
const selectProduct = (product) => {
  ...
  const nameVal = product.displayName || product.name || product.Name || product.DisplayName || 'Sản phẩm'
  const priceVal = product.price || product.Price || 0
  const imgVal = product.coverImageUrl || product.CoverImageUrl || ''
  const newProduct = {
    id: Date.now() + Math.random(),
    product_id: product.id || product.Id,
    code: String(product.id || product.Id).substring(0, 8),
    name: nameVal,
    quantity: 1,
    unitPrice: priceVal,
    total: priceVal,
    costPrice: product.costPrice || product.cost_price || product.CostPrice || 0,
    coverImageUrl: imgVal,
  }
...
const syncLocalData = () => {
  if (props.order) {
    const o = props.order
    const buyerEmail = o.buyerEmail || o.customerEmail || o.customer_email || o.BuyerEmail || ''
    const buyerName = o.buyerName || o.BuyerName || ''
    const buyerId = o.buyerId || o.buyer_id || o.BuyerId
    const phone = o.buyerPhone || o.customerPhone || o.customer_phone || o.BuyerPhone || o.CustomerPhone
    ...
    const productsRaw = o.products || o.Products || o.items || o.Items || o.outputInfos || o.OutputInfos || []
    localData.value.products = productsRaw.map((p) => {
      const prodId = p.productId || p.product_id || p.ProductId || p.productVarientId || p.ProductVarientId
      const prodName = p.productName || p.name || p.ProductName || p.displayName
      const qty = p.count || p.quantity || p.Count || 1
      const priceVal = Number(p.price || p.unitPrice || p.Price || 0)
      const img = p.coverImageUrl || p.cover_image_url || p.CoverImageUrl || p.image
      return {
        id: p.id || Date.now() + Math.random(),
        product_id: prodId,
        code: p.code || p.Code || (prodId ? String(prodId).substring(0, 8) : 'N/A'),
        name: prodName,
        quantity: qty,
        unitPrice: priceVal,
        total: priceVal * qty,
        costPrice: Number(p.costPrice || p.cost_price || p.CostPrice || 0),
        coverImageUrl: img,
      }
    })
    localData.value.shippingFee = o.shippingFee || o.shipping_fee || o.ShippingFee || 0
    localData.value.notes = o.notes || o.Notes || ''
    localData.value.customerName = o.customerName || o.customer_name || o.CustomerName || ''
    localData.value.customerAddress =
      o.customerAddress || o.customer_address || o.CustomerAddress || ''
    localData.value.customerPhone = o.customerPhone || o.customer_phone || o.CustomerPhone || ''
    localData.value.paymentMethod = o.paymentMethod || o.payment_method || o.PaymentMethod || 'COD'
    localData.value.depositRatio =
      o.depositRatio !== undefined && o.depositRatio !== null
        ? o.depositRatio
        : o.deposit_ratio !== undefined && o.deposit_ratio !== null
          ? o.deposit_ratio
          : depositRatioSetting.value
    localStatus.value = o.statusId || o.status_id || o.StatusId || 'pending'
  } else {
    localData.value = {
      customer: null,
      products: [],
...
const totalAmount = computed(() => {
  const productSum = localData.value.products.reduce((sum, p) => sum + (p.total || 0), 0)
  return productSum + (localData.value.shippingFee || 0)
})

const estimatedCommission = computed(() => {
  return localData.value.products.reduce((sum, p) => {
    const productName = p.name || ''
    const isVehicle = productName.toLowerCase().includes('xe') || p.unitPrice >= 10000000
    if (isVehicle) {
      return sum + (p.quantity * 500000)
    } else {
      return sum + (p.total * 0.03)
    }
  }, 0)
})
  ...
  >
                ...
                <div class="flex items-center gap-3">
                  ...
                  <div class="flex-1">
                    ...
                    <div class="text-xs text-gray-500">
                      ID: {{ (product.id || product.Id) || 'N/A' }} | Giá:
    ...
    <template #footer>
      <div class="flex items-center justify-between w-full">
        <div class="flex flex-col gap-1 pl-6 pb-4">
          <div v-if="localData.shippingFee > 0" class="text-sm text-gray-500 font-medium">
            Phí giao hàng: {{ formatVNDWithUnit(localData.shippingFee) }}
          </div>
          <div class="text-xl font-bold text-primary-600">Tổng: {{ formatVNDWithUnit(totalAmount) }}</div>
          <div v-if="estimatedCommission > 0" class="bg-blue-50 text-blue-700 px-3 py-1 mt-1 rounded-lg text-xs font-bold border border-blue-100 flex items-center gap-2 w-fit">
            <font-awesome-icon icon="coins" />
            <span>Hoa hồng dự kiến: {{ formatCurrency(estimatedCommission) }}</span>
          </div>
        </div>
        <div class="flex gap-2 pr-6 pb-4 items-end">
