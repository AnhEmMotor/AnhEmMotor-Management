...
import InventoryIdentificationForm from './InventoryIdentificationForm.vue'
...
<template>
  <div v-if="show">
    <DraggableModal
      ...
      :z-index="1000"
      :width="isVehicleWorkflow ? '80vw' : '70vw'"
    ...
    >
      <template #header>
        <h3 class="text-lg font-semibold">
          {{ isEditMode ? 'Chỉnh sửa phiếu nhập' : (isVehicleWorkflow ? 'Nhập xe mới & Định danh tài sản' : 'Tạo phiếu nhập hàng') }}
        </h3>
      </template>

      <template #body>
        <InventoryIdentificationForm 
          v-if="isVehicleWorkflow"
          v-model="localData"
      ...
      <template v-if="!isVehicleWorkflow" #footer>
