<template>
  <div class="resp-page voucher-detail-page flex flex-col gap-5 pb-10">
    ...
    <ElEmpty
      v-else-if="!voucher"
      description="Không tìm thấy thông tin voucher"
    />
    ...
    <template v-else>
      ...
      >
        ...
        <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
          <div
            class="kpi-card bg-gray-50 dark:bg-slate-800 rounded-xl p-3 text-center"
          >
          ...
          >
            ...
            >
              {{
                voucher.type === "PRIVATE"
                  ? voucher.assignedCustomerIds?.length || 0
                  : "∞"
              }}
            </div>
          </div>
          ...
          <div class="flex flex-col gap-3">
            ...
            >
              <span class="text-sm text-gray-500 dark:text-slate-400"
                >Phạm vi sản phẩm</span
              >
              <div class="flex items-center gap-1.5">
                <ArtSvgIcon
                  :icon="applyForConfig.icon"
                  :class="applyForConfig.color"
                />
                <span
                  class="text-sm font-semibold text-gray-800 dark:text-slate-100"
                  >{{ applyForConfig.label }}</span
                >
              </div>
            </div>
            ...
            >
              ...
              <div class="flex items-center gap-1.5">
                <ArtSvgIcon
                  ...
                  :class="channelConfig.color"
                />
                <span
                  class="text-sm font-semibold text-gray-800 dark:text-slate-100"
                  >{{ channelConfig.label }}</span
                >
              </div>
            </div>
            ...
            >
              <span class="text-sm text-gray-500 dark:text-slate-400"
                >Loại khuyến mãi</span
              >
              <ElTag
                :type="
                  voucher.discountType === 'PERCENT' ? 'warning' : 'success'
                "
                effect="plain"
                round
                size="small"
        ...
        >
          ...
          <div class="flex flex-col gap-3">
            ...
            >
              ...
              <span
                class="text-sm font-semibold text-gray-800 dark:text-slate-100"
                >{{ formatDateTime(voucher.validFrom) }}</span
              >
            </div>
            ...
            <div
              v-else
              class="bg-gray-50 dark:bg-slate-800 rounded-xl p-3 flex items-center gap-2"
            >
      ...
      >
        ...
        <div
          v-if="!voucher.assignedCustomerIds?.length"
...
const daysLeft = computed(() =>
  voucher.value
    ? Math.max(0, dayjs(voucher.value.validTo).diff(dayjs(), "day"))
    : 0,
);
const daysToStart = computed(() =>
  voucher.value
    ? Math.max(0, dayjs(voucher.value.validFrom).diff(dayjs(), "day"))
    : 0,
);
...
const progressPercent = computed(() => {
  ...
  const total = dayjs(voucher.value.validTo).diff(
    dayjs(voucher.value.validFrom),
    "minute",
  );
...
const goEdit = () =>
  router.push(`/Marketing/customer/voucher/save?id=${voucher.value?.id}`);
