      ...
      <div class="flex items-center justify-between">
        <div class="flex items-center gap-4">
          <div class="size-11 rounded-xl bg-[#001529] flex-cc text-white shadow-lg shrink-0">
            <ArtSvgIcon icon="ri:user-heart-line" class="text-2xl" />
          </div>
          <div>
            <h1 class="m-0 text-xl font-bold tracking-tight text-slate-900 leading-none">
              {{ $t('marketing.customer.title') }}
            </h1>
            <p class="m-0 text-[10px] font-bold text-slate-400 uppercase tracking-[0.15em] mt-1.5">
    ...
    <div class="p-6">
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        <div
          ...
          :key="menu.path"
          class="bg-white rounded-lg border border-slate-200 p-4 hover:shadow-md transition-shadow cursor-pointer"
          @click="$router.push(menu.path)"
        >
          <div class="flex items-center gap-3">
            <div class="size-10 rounded-lg bg-blue-50 flex items-center justify-center">
              <ArtSvgIcon :icon="menu.icon" class="text-lg text-blue-600" />
            </div>
            <div>
