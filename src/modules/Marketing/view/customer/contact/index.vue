<template>
  ...
  >
              ...
              <div v-else class="detail-content">
                ...
                <div class="section">
                  ...
                  <div class="content-box">
                    <p class="content-text">{{ activeItemContent }}</p>
                  </div>
                </div>
                <div class="section">
                  ...
                  <div class="note-box">
                    ...
                    <p class="note-text">
                      {{ activeItemInternalNote || "Chưa có ghi chú..." }}
                    </p>
                  </div>
                </div>
                  ...
                  <div class="action-btns">
                    ...
                    >
                      <ArtSvgIcon icon="ri:check-double-line" class="mr-1" />{{
                        $t("contact.statusClosed")
                      }}
                    </ElButton>
                    <ElButton
                      v-if="activeItem && activeItem.status !== 'InProgress'"
                      class="action-btn info"
                      @click="handleStatus('InProgress')"
                    >
                      <ArtSvgIcon icon="ri:loader-4-line" class="mr-1" />{{
                        $t("contact.statusInProgress")
                      }}
                    </ElButton>
                    <ElButton
              ...
              <div v-else class="detail-content">
                ...
                <div class="section">
                  ...
                  <div class="content-box">
                    <p class="content-text">
                      {{ activeItemCoverLetter || "Không có thư xin việc." }}
                    </p>
                  </div>
                </div>
                <div class="section">
                ...
                <div class="actions-section">
                  ...
                  <div class="action-btns">
                    ...
                    >
                      <ArtSvgIcon icon="ri:mic-line" class="mr-1" />{{
                        $t("contact.statusInterview")
                      }}
                    </ElButton>
                    <ElButton
                    ...
                    <ElButton
                      v-if="hasCvUrl"
                      class="action-btn"
                      @click="openCvViewer(activeItem!)"
                    >
                      <ArtSvgIcon icon="ri:file-pdf-2-line" class="mr-1" />{{
                        $t("contact.actionViewCv")
                      }}
                    </ElButton>
                    <ElButton class="action-btn" @click="openAssignDialog">
                      <ArtSvgIcon icon="ri:user-settings-line" class="mr-1" />{{
    ...
    <ElDialog
      ...
      :title="$t('contact.noteDialog.title')"
      width="420px"
    >
      <ElInput
        v-model="noteDraft"
        type="textarea"
        :rows="4"
        :placeholder="$t('contact.noteDialog.placeholder')"
        class="compact-textarea"
      />
      <template #footer>
        <ElButton @click="saveNote">{{
          $t("contact.noteDialog.saveBtn")
        }}</ElButton>
      </template>
    </ElDialog>
    ...
    <!-- CV VIEWER DIALOG -->
    <ElDialog
      v-model="cvDialogVisible"
      :title="$t('contact.cvViewer.title')"
      width="720px"
      top="5vh"
    >
      ...
      <ElForm :model="createForm" label-position="top">
        <template v-if="activeTab === 'support'">
          ...
          <ElFormItem :label="$t('contact.createDialog.category')">
            <ElSelect v-model="createForm.category" class="w-full">
              <ElOption
                v-for="c in supportCategories"
                :key="c"
                :label="$t('contact.category.' + c)"
                :value="c"
              />
            </ElSelect>
          </ElFormItem>
        ...
        <template v-if="activeTab === 'candidate'">
          ...
          <ElFormItem :label="$t('contact.createDialog.appliedPosition')">
            <ElInput
              v-model="createForm.appliedPosition"
              class="compact-input"
            />
          </ElFormItem>
          <ElFormItem :label="$t('contact.createDialog.cvFile')">
            <ElInput
              v-model="createForm.cvFileUrl"
              class="compact-input"
          ...
          <ElFormItem :label="$t('contact.createDialog.coverLetter')">
            <ElInput
              v-model="createForm.coverLetter"
              type="textarea"
              :rows="3"
              class="compact-textarea"
            />
...
const activeItemInternalNote = computed(
  () => (activeItem.value as any)?.internalNote ?? "",
);
...
const openNoteDialog = () => {
  noteDraft.value = activeItemInternalNote.value;
...
const openCvViewer = (row: Contact.ContactItem) => {
  currentCvUrl.value = (row as Contact.JobApplication).cvFileUrl || "";
  cvDialogVisible.value = true;
};
const downloadCv = () => {
  if (currentCvUrl.value) window.open(currentCvUrl.value, "_blank");
};
...
const handleCreate = async () => {
  try {
    if (activeTab.value === "support")
      await ContactApi.createSupportRequest({ ...createForm.value } as any);
    else if (activeTab.value === "feedback")
      await ContactApi.createFeedback({ ...createForm.value } as any);
...
.cv-dialog :deep(.el-dialog__body) {
  padding: 0;
}
