...
export function getLeadStatusLabel(status?: string) {
  return LEAD_STATUS_OPTIONS.find((item) => item.value === status)?.label ?? status ?? 'Chưa có';
}

export function getLeadStatusType(status?: string) {
  return LEAD_STATUS_OPTIONS.find((item) => item.value === status)?.type ?? 'info';
}

export function getOutputStatusLabel(status?: string) {
  return OUTPUT_STATUS_OPTIONS.find((item) => item.value === status)?.label ?? status ?? 'Chưa có';
}
...
export function isHighIntentLeadStatus(status?: string) {
  return status === 'TestDriving' || status === 'Deposited' || status === 'Paperwork';
}
