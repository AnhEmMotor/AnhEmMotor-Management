  ...
  <div class="resp-page hr-policy-container flex flex-col gap-4 h-full p-4">
    ...
    <ElTabs v-model="activeTab" class="department-tabs">
      <ElTabPane name="vehicle_sales">
        <template #label>
          <span class="flex items-center gap-2 text-base font-bold"> 🏍️ SALE XE MÁY </span>
        </template>
      </ElTabPane>
      <ElTabPane name="parts_sales">
        <template #label>
          <span class="flex items-center gap-2 text-base font-bold"> ⚙️ SALE PHỤ TÙNG/ONLINE </span>
        </template>
      </ElTabPane>
      <ElTabPane name="mechanic">
        <template #label>
          <span class="flex items-center gap-2 text-base font-bold"> 🛠️ KỸ THUẬT VIÊN XƯỞNG </span>
        </template>
      </ElTabPane>
    </ElTabs>
    ...
    <div class="list-container flex-1 overflow-y-auto">
      ...
      >
        ...
        >
          ...
          <h3 class="font-bold text-lg mb-2 pr-28 text-gray-800 dark:text-gray-100">
            {{ policy.name }}
          </h3>
          ...
          >
            <template v-if="policy.department === 'mechanic'">
              ...
              <div class="grid grid-cols-2 gap-2">
                <div class="rounded-md bg-white/80 px-2 py-2 dark:bg-gray-900/70">
                  <span class="block text-[11px] text-gray-500 dark:text-gray-400">Tiền công</span>
                  <strong class="text-base text-red-600 dark:text-red-400">
                    {{ formatPercentage(policy.laborPercentage) }}
                  </strong>
                </div>
                <div class="rounded-md bg-white/80 px-2 py-2 dark:bg-gray-900/70">
                  <span class="block text-[11px] text-gray-500 dark:text-gray-400">Phụ tùng</span>
                  <strong class="text-base text-red-600 dark:text-red-400">
          ...
          >
            <span class="text-xs font-medium text-gray-500">Bấm để xem chi tiết</span>
...
const formatPolicyValue = (policy: PolicyValueFields) => {
  ...
  return `${new Intl.NumberFormat('vi-VN').format(safeAmount)} ₫${unit ? ` / ${unit}` : ''}`;
};
...
const getPolicyItemClass = (policy: any) => {
  const base = 'border-gray-200 dark:border-gray-700 hover:border-primary hover:-translate-y-1';
  if (policy.status === 'active') return `${base} border-green-200 dark:border-green-800`;
  if (policy.status === 'pending') return `${base} border-orange-200 dark:border-orange-800`;
...
const mapBackendPolicy = (p: CommissionPolicyResponse) => {
  ...
  const dept: DepartmentKey =
    ...
    (target.includes('Kỹ thuật') || target === 'Mechanic'
      ? 'mechanic'
      : target.includes('Phụ tùng') || target === 'PartsSales' || p.type === 'Percentage'
        ? 'parts_sales'
        : 'vehicle_sales');
  return {
    id: p.id,
    ...
    laborPercentage:
      uiConfiguration.laborPercentage ?? (dept === 'mechanic' ? Number(p.value) || 0 : 0),
