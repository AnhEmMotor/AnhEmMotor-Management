    ...
    <el-card shadow="never" class="reporting-card">
      <template #header>
        <div class="card-header flex justify-between items-center">
          <span class="text-lg font-bold text-slate-800 dark:text-slate-100">{{ $t("menus.hr.payroll") }}</span>
        </div>
      </template>

      <!-- KPI Summary Cards Grid -->
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 mb-6 reporting-kpi-grid">
        <ArtStatsCard
      ...
      <!-- Search & Filters -->
      <div class="bg-slate-50 dark:bg-slate-800/40 p-4 rounded-lg border border-slate-100 dark:border-slate-800 mb-6">
        <ArtSearchBar
          v-model="searchForm"
          :items="searchItems"
          :label-width="120"
      ...
      <!-- Table Card -->
      <ElCard class="flex-1 art-table-card border border-slate-100 dark:border-slate-800">
        <ArtTableHeader
        ...
        >
          <template #fullName="{ row }">
            <span class="font-medium text-slate-700 dark:text-slate-200">{{ row.fullName || "-" }}</span>
          </template>

          <template #baseSalary="{ row }">
            <span class="text-slate-600 dark:text-slate-300">{{ formatCurrency(row.baseSalary) }}</span>
          </template>

          <template #confirmedCommission="{ row }">
            <span class="text-slate-600 dark:text-slate-300 font-semibold">{{ formatCurrency(row.confirmedCommission) }}</span>
          </template>

          <template #volumeBonus="{ row }">
            <div class="flex flex-col items-end">
              <span class="text-slate-600 dark:text-slate-300">{{ formatCurrency(row.volumeBonus) }}</span>
              <ElTag v-if="row.volumeBonus > 0" type="danger" size="small" class="mt-1 bg-red-50 text-red-500 border-red-200 dark:bg-red-950/30 dark:text-red-400 dark:border-red-900/50">Đạt 10 xe</ElTag>
            </div>
          </template>

          <template #totalNetPayable="{ row }">
            <span class="font-semibold text-rose-600 dark:text-rose-400">{{ formatCurrency(row.totalNetPayable) }}</span>
          </template>
          ...
          <template #operation="{ row }">
            <div class="flex gap-2 justify-center">
              ...
              <span v-else class="text-xs text-slate-400 dark:text-slate-500">Đã duyệt / Không có commission</span>
            </div>
          </template>
        </ArtTable>
      </ElCard>
...
const searchItems = ref([
  { key: "month", label: "Tháng", type: "input", props: { placeholder: "VD: 7" } },
...
const columns = ref<ColumnOption[]>([
  ...
  { label: "Lương cơ bản", prop: "baseSalary", width: 140, align: "right", useSlot: true },
  { label: "Hoa hồng chờ chi", prop: "confirmedCommission", width: 160, align: "right", useSlot: true },
  { label: "Thưởng đạt ngưỡng", prop: "volumeBonus", width: 160, align: "right", useSlot: true },
  { label: "Thực nhận", prop: "totalNetPayable", width: 160, align: "right", useSlot: true },
  {
    label: "Thao tác",
    prop: "operation",
    width: 180,
...
const loadData = async () => {
  ...
  try {
    ...
    const totalVolumeBonus = summaryData.reduce((sum, item) => sum + (item.volumeBonus || 0), 0);
    const totalPayrollVal = summaryData.reduce((sum, item) => sum + (item.totalNetPayable || 0), 0);
    const pendingVal = summaryData.reduce(
      (sum, item) => sum + (item.confirmedCommission || 0) + (item.volumeBonus || 0),
      0
    );
