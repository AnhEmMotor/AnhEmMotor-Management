<template>
  <div v-loading="loading" class="resp-page flex flex-col gap-4 pb-5">
    <div class="flex items-center gap-4">
      ...
      <ElTag :type="statusTagType(detail?.paymentStatus || '')" effect="dark" class="ml-2">
    ...
    <div v-if="detail" class="space-y-6 max-w-[800px]">
      <ElCard>
        ...
        <div class="resp-stats-2 grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
          ...
          <div>
            ...
            <b class="text-slate-900 ml-2">{{ getSourceTypeLabel(detail.sourceType) }}</b>
          </div>
      ...
      <ElCard class="bg-red-50/30 border-red-100">
        ...
        <div class="space-y-3">
          ...
          <div class="flex justify-between text-xl font-black border-t border-red-100 pt-3">
            <span>Thành tiền:</span>
            <span class="text-red-600">{{ formatCurrency(detail.totalAmount) }} đ</span>
          </div>
    ...
    <div v-else class="text-sm text-slate-500">Không tìm thấy phiếu thanh toán.</div>
  </div>
</template>
