    ...
    <div class="flex items-start justify-between gap-4 mb-6 flex-wrap">
      <div>
        ...
        <p class="mt-1 text-sm text-slate-500">
          Theo dõi hiệu suất làm việc, tổng doanh số đóng góp và đánh giá dịch vụ của từng kỹ thuật
          viên.
        </p>
      </div>

      <div class="flex items-center gap-2">
        <ElButton :icon="Refresh" type="primary" :loading="loading" @click="loadTechnicians">
          Làm mới dữ liệu
        </ElButton>
      </div>
    </div>
    ...
    <div v-loading="loading">
      ...
      <div v-else class="resp-cards-3 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        >
          ...
          >
            <div>
              <div class="text-[10px] text-slate-400 uppercase font-bold">Phiếu hoàn thành</div>
              <div class="text-lg font-black text-slate-900 mt-0.5">
                {{ tech.completed }}
              </div>
            </div>
            <div>
              <div class="text-[10px] text-slate-400 uppercase font-bold">Doanh thu mang lại</div>
          ...
          <div class="space-y-1.5">
            <div class="flex items-center justify-between text-xs">
              ...
              <span
                class="font-bold"
                :class="tech.complaintRate > 0 ? 'text-red-500' : 'text-success'"
              >
...
const loadTechnicians = async () => {
  loading.value = true;
  try {
    const res = await statisticsApi.getWorkshopDashboardOverview().catch(() => null);
