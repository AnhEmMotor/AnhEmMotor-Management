  ...
  <div class="resp-page p-4 dashboard-workshop">
      ...
      <div>
        <div class="flex items-center gap-3">
          ...
          >
            ...
            <span class="text-xs font-semibold tracking-wide">LIVE / THỜI GIAN THỰC</span>
          </div>
        </div>
        <p class="mt-1 text-sm text-g-500">
          Dashboard quản lý xưởng: KPI tiến độ, cảnh báo phiếu quá hạn/vật tư và analytics doanh thu
          dịch vụ.
        </p>
      </div>
    ...
    <div class="mt-6 grid grid-cols-1 gap-4 lg:grid-cols-3 items-start">
        ...
        <div v-else class="space-y-3">
          ...
          >
            <div class="flex items-center justify-between gap-3">
              <div class="text-sm font-semibold text-g-900">#{{ o.ticketId }}</div>
              <ElTag type="danger" effect="dark"> Quá hạn: {{ o.overdueHours }} giờ </ElTag>
            </div>
            <div class="mt-2 text-xs text-g-500">
              Khách hàng:
              <span class="font-medium text-g-700">{{ o.customerName }}</span>
            </div>
            <div class="mt-1 text-xs text-g-500">Trạng thái: {{ o.status }}</div>
          </div>
        </div>
      </ElCard>
      ...
      <ElCard class="lg:col-span-1">
        ...
        <div v-if="alerts.partsShortage.length === 0" class="text-sm text-g-500">
          Không thiếu vật tư.
        </div>
        ...
        <div v-else class="space-y-3">
          ...
          >
            <div class="flex items-center justify-between gap-3">
              <div class="text-sm font-semibold text-g-900">#{{ p.ticketId }}</div>
              <div class="flex gap-2">
                <ElTag type="danger" effect="dark">Thiếu</ElTag>
                <ElButton size="small" type="primary" link>Đặt hàng bổ sung</ElButton>
              </div>
            </div>
            <div class="mt-1 text-xs text-g-500">Hạng mục: {{ p.partName }}</div>
      ...
      <ElCard class="lg:col-span-1" :class="{ 'mt-0': true }">
        ...
        <div v-if="warrantyAndComplaints.loading" class="text-sm text-g-500">Đang tải...</div>
        <div v-else>
          <div class="grid grid-cols-1 gap-3">
            ...
            >
              <div class="flex items-center justify-between gap-3">
                <div class="text-sm font-semibold text-g-900">Yêu cầu bảo hành kỹ thuật</div>
                <ElTag type="warning" effect="dark">
                  {{ warrantyAndComplaints.warrantyRequestsCount }}
                </ElTag>
              </div>
              <div class="mt-1 text-xs text-g-500">Ưu tiên xử lý các yêu cầu quá hạn.</div>
            </div>
            ...
            >
              ...
              <div class="mt-1 text-xs text-g-500">Theo dõi mức độ hài lòng và phản hồi.</div>
            </div>
            ...
            <div v-else class="text-center text-sm text-g-400 py-4">Chưa có dữ liệu gần đây.</div>
          </div>
        </div>
      </ElCard>
    </div>
    ...
    <div class="mt-6 grid grid-cols-1 gap-4 lg:grid-cols-3">
      <ElCard class="lg:col-span-2 overflow-hidden relative">
        <template #header>
          <span class="font-semibold">Biểu đồ so sánh doanh thu xưởng theo tháng</span>
        </template>
...
const statusBarChart = computed(() => {
  const dataMap = new Map(analytics.value.repairOrderStatusCounts.map((x) => [x.status, x.count]));
...
const refresh = async () => {
  ...
  try {
    ...
    kpi.value = {
      inProgress: Number(asAny?.KpiCards?.InProgressCount ?? asAny?.kpiCards?.inProgressCount ?? 0),
      avgFinishHours: Number(
        asAny?.KpiCards?.AvgCompletionHours ?? asAny?.kpiCards?.avgCompletionHours ?? 0
      ),
      serviceRevenue: Number(
        asAny?.KpiCards?.CumulativeRevenue ?? asAny?.kpiCards?.cumulativeRevenue ?? 0
      ),
      serviceRevenueVsTargetPct: 0,
    };

    const revComparison =
      asAny?.Analytics?.RevenueComparison ?? asAny?.analytics?.revenueComparison;
    if (revComparison) {
      analytics.value.revenueComparison = {
        workshopRevenue: Number(
          revComparison.WorkshopRevenue ?? revComparison.workshopRevenue ?? 0
        ),
        retailRevenue: Number(revComparison.RetailRevenue ?? revComparison.retailRevenue ?? 0),
      };
    } else {
      analytics.value.revenueComparison = {
        workshopRevenue: 0,
    ...
    const rawRevenueSources =
      asAny?.Analytics?.RevenueSources ?? asAny?.analytics?.revenueSources ?? [];
    ...
    const rawRevenueTrend = asAny?.Analytics?.RevenueTrend ?? asAny?.analytics?.revenueTrend;
    if (rawRevenueTrend) {
      analytics.value.revenueTrend = {
        labels: rawRevenueTrend.Labels ?? rawRevenueTrend.labels ?? [],
        serviceRevenue: rawRevenueTrend.ServiceRevenue ?? rawRevenueTrend.serviceRevenue ?? [],
        retailRevenue: rawRevenueTrend.RetailRevenue ?? rawRevenueTrend.retailRevenue ?? [],
      };
    }

    const rawStatusCounts =
      asAny?.Analytics?.RepairOrderStatusCounts ?? asAny?.analytics?.repairOrderStatusCounts ?? [];
    ...
    const rawOverdue = (asAny?.Alerts?.OverdueTickets ?? asAny?.alerts?.overdueTickets) || [];
    alerts.value.overdue =
      rawOverdue.length > 0
        ? rawOverdue.map((o: any) => {
            const ticketId = o.TicketId ?? o.ticketId ?? 0;
            const expectedCompletionTime = o.ExpectedCompletionTime ?? o.expectedCompletionTime;
            const overdueHours = expectedCompletionTime
              ? Math.max(
                  0,
                  Math.floor(
                    (Date.now() - new Date(expectedCompletionTime).getTime()) / (1000 * 60 * 60)
                  )
                )
              : 0;
            return {
              ...
              overdueHours,
              expectedCompletionTime: expectedCompletionTime ?? new Date().toISOString(),
            };
          })
        : [];

    const rawParts = (asAny?.Alerts?.PartShortages ?? asAny?.alerts?.partShortages) || [];
    ...
    const rawTechnicians =
      (asAny?.Productivity?.TechnicianRankings ?? asAny?.productivity?.technicianRankings) || [];
    technicianRows.value =
      rawTechnicians.length > 0
        ? rawTechnicians.map((t: any) => ({
            ...
            customerSatisfaction: 100 - (t.ComplaintRate ?? t.complaintRate ?? 0) * 100,
          }))
        : [];

    warrantyAndComplaints.value = {
      loading: false,
      warrantyRequestsCount: asAny?.WarrantyRequestsCount ?? asAny?.warrantyRequestsCount ?? 0,
