    ...
    >
      <ElForm :model="bookingForm" label-width="100px">
        ...
        <ElFormItem label="Xe lái thử" v-if="bookingForm.type === 'TestDrive'">
          <ElSelect
            v-model="bookingForm.productVariantId"
        ...
        <ElFormItem
          :label="
            bookingForm.type === 'TestDrive' ? 'Ghi chú nội bộ' : 'Nội dung'
          "
        >
          <ElInput
        ...
        <div v-if="bookingForm.status === 'Pending' && isEditing" class="mt-2">
          <div class="p-3 bg-blue-50 rounded-2xl border border-blue-100">
            <p
              class="m-0 text-[12px] font-bold text-blue-700 leading-normal mb-1"
            >
            ...
            <p class="m-0 text-[10px] text-blue-700 leading-normal italic">
              ...
              <strong>Đang lái thử</strong> và hệ thống gửi Email/SMS xác nhận
              cho khách.
            </p>
          </div>
        </div>
        <div
          v-else-if="bookingForm.status === 'Confirmed' && isEditing"
          class="mt-2"
        >
          <div class="p-3 bg-success-50 rounded-2xl border border-success-100">
            <p
              class="m-0 text-[12px] font-bold text-success-700 leading-normal"
            >
              Trạng thái Pipeline:
              <ElTag size="small" type="success">Đang lái thử</ElTag>
            </p>
          </div>
        </div>
      </ElForm>
      ...
      <template #footer>
        <span class="dialog-footer">
          <ElButton v-if="isEditing" type="danger" :disabled="true"
            >Xóa (chưa hỗ trợ)</ElButton
          >
          ...
          <ElButton v-if="!isEditing" type="primary" @click="handleSaveEvent"
            >Tạo mới</ElButton
          >
...
const { getBookingEvents, confirmBooking, createBooking } =
