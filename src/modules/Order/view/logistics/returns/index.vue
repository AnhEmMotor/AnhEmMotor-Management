<template>
  <div class="resp-page returns-page flex flex-col gap-4 animate__animated animate__fadeIn">
    <ElCard class="shrink-0" body-class="pb-0">
      ...
      <ElTabs v-model="statusFilter" @tab-change="fetchReturns" class="order-tabs">
    ...
    <div class="flex-1 h-[calc(100vh-220px)] mt-2">
      ...
      >
        ...
        <ElTableColumn prop="carrier" label="Vận chuyển" width="150" align="center">
          <template #default="{ row }">
            <ElTag :type="getCarrierTag(row.carrier)" size="small" effect="plain">{{
              row.carrier
            }}</ElTag>
          </template>
        </ElTableColumn>
        <ElTableColumn prop="reason" label="Lý do hoàn" min-width="200" show-overflow-tooltip>
          <template #default="{ row }">
            <ElTag :type="getReasonTag(row.reason)" size="small">{{ row.reason }}</ElTag>
          </template>
        </ElTableColumn>
        <ElTableColumn prop="status" label="Trạng thái" width="150" align="center">
          <template #default="{ row }">
