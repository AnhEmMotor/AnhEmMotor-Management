<template>
  <div class="warehouse-report">
    ...
    <ElCard class="reporting-card mt-4">
      ...
      <ElTable :data="data.warehouseTableData" class="reporting-table" empty-text="Chưa có dữ liệu">
        <ElTableColumn prop="brandName" label="Thương hiệu" min-width="180">
          <template #default="{ row }">{{ row.brandName || '-' }}</template>
        </ElTableColumn>
        <ElTableColumn prop="totalStock" label="Tổng tồn" min-width="110" align="right" />
        <ElTableColumn prop="capacity" label="Sức chứa" min-width="110" align="right" />
        <ElTableColumn prop="lowStock" label="Sắp hết" min-width="110" align="right">
          <template #default="{ row }">
            <span class="text-yellow-600">{{ row.lowStock }}</span>
          </template>
        </ElTableColumn>
        <ElTableColumn prop="outOfStock" label="Hết hàng" min-width="110" align="right">
          <template #default="{ row }">
            <span class="text-red-600">{{ row.outOfStock }}</span>
          </template>
        </ElTableColumn>
        <ElTableColumn prop="value" label="Giá trị" min-width="150" align="right">
        ...
        <ElTableColumn prop="status" label="Trạng thái" min-width="130" align="center">
          <template #default="{ row }">
            <ElTag :type="warehouseStatusType(row.status)" size="small" effect="light" round>
              {{ row.status || '-' }}
            </ElTag>
          </template>
        </ElTableColumn>
...
<script setup lang="ts">
  ...
  const data = ref<Statistical.AdminWarehouseReportResponse>({
    summary: { totalStock: 0, totalValue: 0, lowStockCount: 0, outOfStockCount: 0 },
    stockByBrand: [],
    stockStatusRatio: [],
    warehouseTableData: [],
  })
  ...
  const stockRatio = computed(() => {
    ...
    return (
      (((s.totalStock - s.lowStockCount - s.outOfStockCount) / s.totalStock) * 100).toFixed(1) + '%'
    )
  })
        ...
        series: [
          {
            type: 'pie',
            radius: ['40%', '70%'],
            data: top.map((d) => ({ name: d.brandName || 'Khác', value: d.stockCount })),
          },
        ],
      })
    }
