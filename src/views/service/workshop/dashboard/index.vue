  ...
  <div class="p-4 dashboard-workshop">
    <div class="flex items-start justify-between gap-4 flex-wrap">
      <div>
        <div class="flex items-center gap-3">
          <h1 class="text-2xl font-bold">{{ $t('menus.service.workshop.dashboard') }}</h1>
          <div
    ...
    <div class="mt-6 grid grid-cols-1 gap-4 lg:grid-cols-3">
        ...
        <div v-else class="space-y-3">
          ...
          >
            ...
            <div class="mt-2 text-xs text-slate-500">
              Khách hàng: <span class="font-medium text-slate-700">{{ o.customerName }}</span>
            </div>
            <div class="mt-1 text-xs text-slate-500">Trạng thái: {{ o.status }}</div>
          </div>
        </div>
      </ElCard>
      ...
      <ElCard class="lg:col-span-1">
        ...
        <div v-if="alerts.partsShortage.length === 0" class="text-sm text-slate-500">
          Không thiếu vật tư.
        </div>
        ...
        <div v-else class="space-y-3">
          ...
          >
            ...
            <div class="mt-1 text-xs text-slate-500">Hạng mục: {{ p.partName }}</div>
      ...
      <ElCard class="lg:col-span-1" :class="{ 'mt-0': true }">
        ...
        <div v-if="warrantyAndComplaints.loading" class="text-sm text-slate-500">Đang tải...</div>
        <div v-else>
          <div class="grid grid-cols-1 gap-3">
            <div class="rounded-lg border border-slate-200 p-3">
              <div class="flex items-center justify-between gap-3">
                <div class="text-sm font-semibold">Yêu cầu bảo hành kỹ thuật</div>
                <ElTag type="warning" effect="dark">
                  {{ warrantyAndComplaints.warrantyRequestsCount }}
                </ElTag>
              </div>
              <div class="mt-1 text-xs text-slate-500">Ưu tiên xử lý các yêu cầu quá hạn.</div>
            </div>

            <div class="rounded-lg border border-slate-200 p-3">
              <div class="flex items-center justify-between gap-3">
                <div class="text-sm font-semibold">Đánh giá &amp; khiếu nại từ khách</div>
                <ElTag type="danger" effect="dark">
                  {{ warrantyAndComplaints.complaintsCount }}
                </ElTag>
              </div>
              <div class="mt-1 text-xs text-slate-500">Theo dõi mức độ hài lòng và phản hồi.</div>
            </div>
    ...
    <div class="mt-6 grid grid-cols-1 gap-4 lg:grid-cols-3">
      <ElCard class="lg:col-span-2 overflow-hidden relative">
        <template #header>
          <span class="font-semibold">Doanh thu dịch vụ vs doanh thu bán xe</span>
        </template>
      ...
      series: [
        {
          name: 'Số lượng',
          data: [
            { value: mock.pending, itemStyle: { color: 'var(--el-color-warning)' } },
            { value: mock.inProgress, itemStyle: { color: 'var(--el-color-primary)' } },
            { value: mock.qcPending, itemStyle: { color: 'var(--el-color-info)' } },
            { value: mock.completed, itemStyle: { color: 'var(--el-color-success)' } },
            { value: mock.cancelled, itemStyle: { color: 'var(--el-text-color-placeholder)' } },
          ],
        },
      ],
    }
  ...
  const refresh = async () => {
    ...
    try {
      ...
      alerts.value = {
        ...
        partsShortage: (asAny?.partsShortage as PartsShortageAlert[]) ?? mock.partsShortage,
      }

      technicianRows.value = (asAny?.technicianPerformance as any[]) ?? mock.technicians
    } finally {
      loading.value = false
    }
  }
