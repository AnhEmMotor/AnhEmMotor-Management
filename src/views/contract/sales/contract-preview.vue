...
<script setup lang="ts">
  ...
  // Dữ liệu mẫu (sẽ được fetch từ API thông qua OutputId)
  const contractData = ref({
    id: '123e4567-e89b-12d3-a456-426614174000',
    contractNumber: 'HDMB-20260530-001',
    status: 'Draft', // Draft, Signed, Fulfilled

    showroomName: 'Đại lý AnhEmMotor',
    showroomTaxCode: '0123456789',
    showroomAddress: '123 Đường Vành Đai, Quận 1, TP.HCM',
    showroomRepresentative: 'Nguyễn Văn A - Giám Đốc',

    customerFullName: 'Trần Thị B',
    customerCCCD: '079192001234',
    customerAddress: '456 Lê Lợi, Phường Bến Nghé, Quận 1, TP.HCM',
    customerPhone: '0901234567',

    vehicleModel: 'Honda SH',
    vehicleVersion: '160i ABS 2026',
    vehicleColor: 'Trắng Đen',
    frameNumber: 'RLH1234567890',
    engineNumber: 'JF123E-0987654',
    actualSalePrice: 105000000,

    depositAmount: 10000000,
    remainingAmount: 95000000,
    finalPaymentDeadline: '2026-06-05',

    warrantyPeriod: '3 năm hoặc 30.000km',
    warrantyScope: 'Theo tiêu chuẩn nhà sản xuất tại tất cả các trạm uỷ nhiệm.',
    specialTerms: 'Tặng kèm 1 mũ bảo hiểm Fullface và 1 lần thay nhớt miễn phí.',
  ...
  // Các bước tiến độ: 0 = Đặt cọc, 1 = Thanh toán đủ, 2 = Bàn giao
  const activeStep = computed(() => {
    if (contractData.value.status === 'Fulfilled') return 3
    if (contractData.value.remainingAmount <= 0) return 1
    return 0
  })
  ...
  const customUploadRequest = (options: any) => {
    setTimeout(() => {
      handleUploadSuccess({ url: '/mock-url' }, options.file)
      options.onSuccess()
    }, 1000)
  }
  ...
  const handleUploadSuccess = (response: any, file: any) => {
    ...
    contractData.value.scannedFileUrl = '/uploads/contracts/' + file.name
