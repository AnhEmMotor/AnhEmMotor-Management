          ...
          <div class="a4-preview-container p-6" style="height: calc(100% - 40px); overflow-y: auto">
            ...
            >
              ...
              <div class="section mb-4">
                ...
                <p
                  ><strong>Đại diện pháp luật:</strong> {{ contractData.showroomRepresentative }}</p
                >
              </div>
              ...
              <div class="section mb-4">
                ...
                <table class="w-full border-collapse border mb-2">
                  <tr>
                    <td class="border p-1"
                      ><strong>Dòng xe:</strong> {{ contractData.vehicleModel }}</td
                    >
                    <td class="border p-1"
                      ><strong>Phiên bản:</strong> {{ contractData.vehicleVersion }}</td
                    >
                  </tr>
                  <tr>
                    <td class="border p-1"
                      ><strong>Màu sắc:</strong> {{ contractData.vehicleColor }}</td
                    >
                    <td class="border p-1"
                      ><strong>Số khung:</strong> {{ contractData.frameNumber }}</td
                    >
                  </tr>
                  <tr>
                    <td class="border p-1" colspan="2"
                      ><strong>Số máy:</strong> {{ contractData.engineNumber }}</td
                    >
                  </tr>
                </table>
                <p
                  ><strong>Đơn giá bán:</strong>
                  {{ formatCurrency(contractData.actualSalePrice) }}</p
                >
              </div>
              ...
              <div class="section mb-4">
                ...
                <p
                  ...
                  {{ formatCurrency(contractData.depositAmount) }}</p
                >
                <p
                  ><strong>Số tiền còn lại:</strong>
                  {{ formatCurrency(contractData.remainingAmount) }}</p
                >
                <p
                  ><strong>Hạn thanh toán cuối:</strong>
                  {{ formatDate(contractData.finalPaymentDeadline) }}</p
                >
              </div>
              ...
              <div class="section mb-4">
                ...
                <p v-if="contractData.specialTerms"
                  ><strong>Cam kết bổ sung:</strong> {{ contractData.specialTerms }}</p
                >
              </div>
...
<style scoped lang="scss">
  ...
  .upload-zone {
    ...
    text-align: center;
    background-color: var(--default-box-color);
    border: 1px dashed rgb(var(--art-color-rgb), 20%);
    border-radius: 6px;
  ...
  :global(html.dark) {
    ...
    .contract-preview-container :deep(.el-card) {
      background: #161618 !important;
      border-color: rgb(255 255 255 / 9%) !important;
    }

    .contract-preview-container :deep(.el-card__header),
    .contract-preview-container :deep(.el-card__body) {
      background: #161618 !important;
    }

    .contract-preview-container :deep(.el-input__wrapper),
    .contract-preview-container :deep(.el-select__wrapper),
    .contract-preview-container :deep(.el-textarea__inner),
    .contract-preview-container :deep(.el-upload-dragger) {
      color: #f8fafc;
    ...
    .contract-preview-container :deep(.el-input__inner),
    .contract-preview-container :deep(.el-select__placeholder),
    .contract-preview-container :deep(.el-select__selected-item),
    .contract-preview-container :deep(.el-textarea__inner) {
      color: #f8fafc !important;
    }

    .contract-preview-container :deep(.el-input__inner::placeholder),
    .contract-preview-container :deep(.el-textarea__inner::placeholder) {
      color: #9ca3af !important;
    }

    .contract-preview-container :deep(.el-upload__text),
    .contract-preview-container :deep(.el-upload__tip) {
      color: #9ca3af !important;
    }
    ...
    .pipeline-card :deep(.text-gray-500) {
      color: #9ca3af !important;
    }

    .pipeline-card :deep(.text-gray-400) {
      color: #6b7280 !important;
    }

    .pipeline-card :deep(.bg-gray-200) {
    ...
    .form-card :deep(.el-card__header) {
      background: #161618 !important;
      border-bottom-color: rgb(255 255 255 / 9%) !important;
    }
    ...
    .upload-zone :deep(.text-gray-500) {
      color: #9ca3af !important;
    }
    ...
    .preview-toolbar :deep(.text-gray-600) {
