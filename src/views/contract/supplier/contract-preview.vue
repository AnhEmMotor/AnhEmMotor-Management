  ...
  <div class="supplier-contract-detail">
    ...
    <el-card shadow="never" class="mb-4">
      <div class="flex justify-between items-center">
        <div>
          <div class="text-sm text-gray-500 mb-1">Chi tiết Hợp đồng Nhà cung cấp</div>
          <div class="flex items-center gap-2">
            <el-tag :type="contractStatusType" size="large">
              {{ getStatusLabel(contract.status) }}
            </el-tag>
            <el-tag v-if="expirationWarningType" :type="expirationWarningType" size="large">
          ...
          <div v-if="daysUntilExpiry > 0" class="mt-2">
            <el-progress
              ...
              style="width: 300px"
            />
            <span class="text-xs text-gray-400 ml-2">Còn {{ daysUntilExpiry }} ngày</span>
          </div>
        </div>
        <div class="flex gap-2">
          ...
          <ElButton @click="handlePrint" :disabled="!contract.id">In hợp đồng</ElButton>
    ...
    <el-alert v-if="showExpirationAlert" type="warning" :closable="false" show-icon class="mb-4">
      <template #title>
        <div class="flex justify-between items-center w-full">
          <span>Cảnh báo: Hợp đồng sắp hết hạn (còn {{ daysUntilExpiry }} ngày)</span>
    ...
    <el-row :gutter="20">
      ...
      <el-col :span="17">
        ...
        <el-card shadow="never" class="mb-4">
          ...
          <el-descriptions :column="2" border size="small">
            <el-descriptions-item label="Tên NCC">{{ supplierName || '-' }}</el-descriptions-item>
            <el-descriptions-item label="Mã NCC">{{ supplierCode || '-' }}</el-descriptions-item>
        ...
        <el-card shadow="never" class="mb-4">
          ...
          <el-descriptions :column="2" border size="small">
            <el-descriptions-item label="Hạn mức công nợ tối đa">
              <span class="font-bold text-primary">{{ formatCurrency(creditLimit) }}</span>
            </el-descriptions-item>
            <el-descriptions-item label="Thời hạn thanh toán">
              {{ paymentWindowDays ? paymentWindowDays + ' ngày' : '-' }}
            </el-descriptions-item>
            <el-descriptions-item label="Ngân hàng">{{ bankName || '-' }}</el-descriptions-item>
        ...
        <el-card shadow="never" class="mb-4">
          ...
          <div class="grid grid-cols-2 gap-4 mb-4">
            <div>
              <span class="text-sm text-gray-500">Sản lượng tối thiểu/tháng:</span>
              <div class="font-medium">{{
                minimumMonthlyVolume ? minimumMonthlyVolume + ' sản phẩm' : '-'
              }}</div>
            </div>
            <div>
          ...
          <ElTable :data="filteredSkuList" border stripe size="small" max-height="400">
            <ElTableColumn prop="skuCode" label="Mã SKU" width="140" />
            <ElTableColumn prop="productName" label="Tên sản phẩm" min-width="180" />
            <ElTableColumn prop="category" label="Danh mục" width="150" />
            <ElTableColumn prop="wholesalePrice" label="Giá nhập sỉ" width="140" align="right">
          ...
          <div v-if="skuList.length === 0" class="text-center text-gray-400 text-sm py-4">
            Chưa có dữ liệu bảng giá sỉ. Hợp đồng ở trạng thái Draft chưa có hiệu lực.
          </div>
        </el-card>
        ...
        <el-card shadow="never" class="mb-4">
          ...
          <el-descriptions :column="2" border size="small">
            ...
            <el-descriptions-item label="Ngày hết hạn">
              {{ contract.expirationDate ? formatDate(contract.expirationDate) : 'Không xác định' }}
            </el-descriptions-item>
        ...
        <el-card shadow="never" class="mb-4">
          ...
          <div v-if="contract.contractFilePath">
            <div class="mb-3">
              ...
              <span class="text-sm text-gray-500 ml-2">{{ contractFileInfo.name }}</span>
            </div>
            <div class="flex gap-2">
          ...
          <div v-else class="text-center py-6">
            ...
            <p class="text-gray-400 text-sm">Chưa tải lên file scan/PDF hợp đồng</p>
            <ElUpload
      ...
      <el-col :span="7">
        ...
        <el-card shadow="never" class="mb-4">
          ...
          <div v-if="contract.contractFilePath" class="text-center">
            <div class="bg-gray-50 rounded-lg p-6 mb-3">
              ...
              <p class="text-sm mt-2">{{ contractFileInfo.name || 'Hợp đồng NCC' }}</p>
            </div>
            <ElButton type="primary" size="small" @click="handleViewFile" class="w-full">
              Mở trình xem PDF
            </ElButton>
          </div>
        ...
        <el-card shadow="never">
          ...
          <div v-if="auditLogs.length > 0" class="space-y-2 max-h-300 overflow-y-auto">
            <div
            ...
            >
              ...
              <p v-if="log.details" class="text-gray-500 text-xs mt-1">{{ log.details }}</p>
              <p class="text-xs text-gray-400 mt-1">{{ log.changedBy }}</p>
            </div>
          </div>
          <div v-else class="text-sm text-gray-400 text-center py-4"> Chưa có nhật ký </div>
        </el-card>
      </el-col>
    </el-row>
    ...
    <ElDialog v-model="showStatusDialog" title="Cập nhật trạng thái hợp đồng" width="400px">
      ...
      <template #footer>
        ...
        <ElButton type="primary" :loading="updatingStatus" @click="handleUpdateStatus"
          >Cập nhật</ElButton
        >
      </template>
    </ElDialog>
...
<script setup lang="ts">
  ...
  const showExpirationAlert = computed(() => {
    return expirationWarningType.value === 'warning' || expirationWarningType.value === 'danger'
  })
  ...
  const bankAccountNumber = computed(() => contract.value.bankAccountNumber || '')
  const minimumMonthlyVolume = computed(() => contract.value.minimumVolumePerMonth || 0)
  ...
  const contractFileInfo = computed(() => {
    ...
    const name = path.split('/').pop() || path.split('\\').pop() || 'contract-file'
  ...
  const handleSoftDelete = async () => {
    ...
    try {
      await ElMessageBox.confirm(
        ...
        'Xác nhận xóa mềm',
        { confirmButtonText: 'Xóa mềm', cancelButtonText: 'Hủy', type: 'warning' },
      )
