...
const tokenGroupKeys: TokenGroupKey[] = ['customer', 'vehicle', 'finance', 'supplier']
...
export const contractTemplateTokens: TokenGroupMap = {
  customer: [
    ...
    { key: '{{ShowroomAddress}}', label: 'Địa chỉ showroom', group: 'customer' },
    { key: '{{ShowroomPhone}}', label: 'SĐT showroom', group: 'customer' },
    { key: '{{ShowroomRepresentative}}', label: 'Đại diện showroom', group: 'customer' },
    { key: '{{ShowroomRepresentativeTitle}}', label: 'Chức danh đại diện', group: 'customer' },
    { key: '{{ShowroomRepresentativeIdCard}}', label: 'CCCD đại diện', group: 'customer' },
    {
    ...
    { key: '{{ShowroomBankAccount}}', label: 'Tài khoản showroom', group: 'customer' },
    { key: '{{ShowroomBankName}}', label: 'Ngân hàng showroom', group: 'customer' },
    ...
    { key: '{{CustomerRepresentativeTitle}}', label: 'Chức danh bên mua', group: 'customer' },
    { key: '{{CustomerIdIssuedDate}}', label: 'Ngày cấp CCCD bên mua', group: 'customer' },
    { key: '{{CustomerIdIssuedPlace}}', label: 'Nơi cấp CCCD bên mua', group: 'customer' },
    { key: '{{CustomerFax}}', label: 'Fax bên mua', group: 'customer' },
    { key: '{{CustomerBankAccount}}', label: 'Tài khoản bên mua', group: 'customer' },
    { key: '{{CustomerBankName}}', label: 'Ngân hàng bên mua', group: 'customer' },
  ],
  vehicle: [
    ...
    { key: '{{PaymentMethod}}', label: 'Hình thức thanh toán', group: 'vehicle' },
    { key: '{{DeliveryDate}}', label: 'Thời gian giao hàng', group: 'vehicle' },
    { key: '{{DeliveryLocation}}', label: 'Địa điểm giao hàng', group: 'vehicle' },
    {
      key: '{{TransportationResponsibleParty}}',
      label: 'Bên chịu vận chuyển',
      group: 'vehicle',
    },
    { key: '{{StorageFeePerDay}}', label: 'Phí lưu kho/ngày', group: 'vehicle' },
    { key: '{{InspectionAgency}}', label: 'Đơn vị kiểm tra', group: 'vehicle' },
    { key: '{{ComplaintDeadlineDays}}', label: 'Hạn khiếu nại', group: 'vehicle' },
    { key: '{{SellerResponseDays}}', label: 'Hạn phản hồi bên bán', group: 'vehicle' },
    { key: '{{WarrantyProductName}}', label: 'Hàng hóa bảo hành', group: 'vehicle' },
    ...
    { key: '{{EachPartyCopies}}', label: 'Số bản mỗi bên giữ', group: 'vehicle' },
  ],
  finance: [
    { key: '{{DepositAmount}}', label: 'Tiền cọc', group: 'finance' },
    { key: '{{RemainingAmount}}', label: 'Tiền còn lại', group: 'finance' },
    { key: '{{FinancialPartner}}', label: 'Đối tác tài chính', group: 'finance' },
    { key: '{{FinancialPartnerContact}}', label: 'Liên hệ tài chính', group: 'finance' },
    { key: '{{FinanceContractNumber}}', label: 'Số HĐ tài chính', group: 'finance' },
    ...
    { key: '{{ExpectedDisbursementDate}}', label: 'Ngày GN dự kiến', group: 'finance' },
    { key: '{{ActualDisbursementDate}}', label: 'Ngày GN thực tế', group: 'finance' },
    {
      key: '{{ExpectedDisbursementAmount}}',
      label: 'Tiền GN dự kiến',
      group: 'finance',
    },
    { key: '{{ActualDisbursementAmount}}', label: 'Tiền GN thực tế', group: 'finance' },
  ...
  supplier: [
    ...
    { key: '{{AppendixChangeContent}}', label: 'Nội dung phụ lục', group: 'supplier' },
  ],
}
...
const tokenByKey = new Map(allContractTemplateTokens.map((token) => [token.key, token]))
...
export const getTokenGroupsForDisplay = (groups: TokenGroupMap = getFullTokenGroups()) =>
  contractTemplateTokenGroupMeta.map((group) => ({
    ...group,
    tokens: groups[group.key] || [],
  }))

export const filterTokenGroups = (groups: TokenGroupMap, searchValue: string) => {
  ...
  tokenGroupKeys.forEach((key) => {
    filtered[key] = groups[key].filter(
      (token) =>
        token.key.toLowerCase().includes(search) || token.label.toLowerCase().includes(search),
    )
  })
...
export const parseDynamicFields = (dynamicFields?: unknown): TokenGroupMap => {
  ...
  const pushToken = (rawToken: unknown, fallbackGroup?: TokenGroupKey) => {
    ...
    const group = isTokenGroupKey(token.group) ? token.group : fallbackGroup || knownToken?.group
    if (!group) return

    groups[group].push({
      key: token.key,
      label: typeof token.label === 'string' ? token.label : knownToken?.label || token.key,
      group,
    })
  }
  ...
  return tokenGroupKeys.some((key) => groups[key].length > 0) ? groups : getFullTokenGroups()
}
...
export const getUnresolvedPreviewTokens = (content = '') => {
  // Without demo data, all tokens are considered unresolved
  // Return all tokens found in the content
...
export const buildPreviewHtml = (content = '') => {
  ...
  // Note: Preview mode shows placeholder tokens since no demo data is available.
  // All tokens remain unresolved and will be highlighted in the UI.
  html = html.replace(
    tokenPattern,
