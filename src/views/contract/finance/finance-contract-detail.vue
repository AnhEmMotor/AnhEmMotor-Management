<template>
  <div class="finance-contract-detail">
    ...
    <!-- KPI Cards Row -->
    <div class="reporting-kpi-grid mb-2 grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
      <ArtStatsCard
    ...
    <!-- Progress Pipeline -->
    <el-card shadow="never" class="mb-2 pipeline-card" body-style="padding: 12px 16px;">
      <div class="text-sm text-gray-500 mb-2 font-medium">Trạng thái vòng đời giải ngân</div>
      ...
      <div class="relative flex w-full overflow-x-auto pb-2">
        ...
        >
          <div
            ...
            :class="[
              pipelineActiveStep > index ? 'border-report-red text-report-red' : '',
              pipelineActiveStep === index ? 'border-report-red bg-report-red text-white' : '',
    ...
    <el-row :gutter="12" class="responsive-layout">
      <!-- Left -->
      <el-col :span="10">
        <el-card shadow="never" class="form-card" body-style="padding: 12px; min-height: 420px;">
          <template #header>
            <div class="card-header font-bold py-2">Thông tin Finance Contract</div>
          </template>

          <el-descriptions :column="1" border size="small" :content-external-class="'py-1 px-2'">
          ...
          <!-- Upload evidence -->
          <div class="mb-2">
            <h4 class="mb-1 font-bold text-sm">Chứng từ đối soát chuyển khoản giải ngân</h4>
            <p class="text-xs text-gray-500 mb-1">
              Tải lên ảnh/bản scan ủy nhiệm chi/bill chuyển khoản từ công ty tài chính.
            </p>
            <el-upload
              drag
          ...
          <div class="mb-2">
            ...
            <el-radio-group v-model="cavetForm.state" class="mt-1">
              <el-radio label="FinancialCompanyHolds" class="mb-1">Công ty tài chính giữ</el-radio>
              <el-radio label="StoreHoldsOnBehalf" class="mb-1">Cửa hàng giữ hộ</el-radio>
              <el-radio label="DeliveredToCustomer" class="mb-1">Đã giao khách</el-radio>
            </el-radio-group>
          ...
          <!-- Validation card -->
          <div class="mt-2">
            <h4 class="mb-1 font-bold text-sm">Đối soát giải ngân (Validation Card)</h4>
            <p class="text-xs text-gray-500 mb-1">
            ...
            <div class="mt-1 text-sm" :class="isAmountMatched ? 'text-green-600' : 'text-red-600'">
              {{
                isAmountMatched
                  ? 'Khớp lệnh 100% ✓'
      ...
      <el-col :span="14">
        ...
        >
          ...
          <div class="a4-preview-container p-2 md:p-3" style="min-height: calc(100% - 36px)">
            <div
              class="a4-paper bg-white shadow-lg p-2 md:p-3 mx-auto"
              style="width: 210mm; max-width: 100%; min-height: 297mm"
            >
              <h2 class="text-center font-bold text-base mb-1">HỢP ĐỒNG TÀI CHÍNH TRẢ GÓP</h2>
              <p class="text-right italic text-xs mb-2">Số: {{ contract.contractNumber || '-' }}</p>

              <div class="section mb-2">
                <h3 class="font-bold text-sm mb-1">THÔNG TIN KHÁCH HÀNG</h3>
                <p class="text-sm"
                  ><strong>Họ tên:</strong> {{ contract.customer360?.fullName || '-' }}</p
                >
                <p class="text-sm"
                  ><strong>CCCD:</strong> {{ contract.customer360?.cccd || '-' }}</p
                >
                <p class="text-sm"
                  ><strong>Địa chỉ:</strong> {{ contract.customer360?.address || '-' }}</p
                >
              </div>

              <div class="section mb-2">
                <h3 class="font-bold text-sm mb-1">THÔNG TIN ĐỐI TÁC TÀI CHÍNH</h3>
                <p class="text-sm"
                  ><strong>Công ty:</strong> {{ contract.financialPartner?.name || '-' }}</p
                >
                <p class="text-sm">
              ...
              <div class="section mb-2">
                ...
                <p class="text-sm">
                  <strong>Kỳ hạn:</strong> {{ contract.creditPackage?.termMonths ?? '-' }} tháng
                </p>
                <p class="text-sm">
              ...
              <div class="section mb-2">
                ...
                <p class="text-sm">
                  <strong>Ngày thực tế:</strong> {{ formatDate(contract.disbursement?.actualDate) }}
                </p>
                <p class="text-sm">
              ...
              <div class="section mb-4">
                ...
                <p v-if="cavetForm.state === 'StoreHoldsOnBehalf'">
                  <strong>Ngày nhận:</strong> {{ formatDateShort(cavetForm.receivedDate) }}
                </p>
                <p v-if="cavetForm.state === 'StoreHoldsOnBehalf'">
                  <strong>Người nhận:</strong> {{ cavetForm.receiverName || '-' }}
                </p>
                <p v-if="cavetForm.state === 'StoreHoldsOnBehalf'">
                  <strong>Vị trí lưu:</strong> {{ cavetForm.storageLocation || '-' }}
                </p>
              </div>
...
<script setup lang="ts">
  ...
  const fetchDetail = async () => {
    ...
    try {
      ...
      ElMessage.error(
        error?.response?.data?.message ?? 'Không tải được chi tiết hợp đồng tài chính.',
      )
    }
  }
