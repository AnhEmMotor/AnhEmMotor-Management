<template>
  <div class="reporting-page contract-template-editor-page">
    <ReportPageHeader
      :title="
        isEditMode ? t('menus.contract.templateEditTitle') : t('menus.contract.templateCreateTitle')
      "
    ...
    <ElCard class="reporting-card mb-5">
      <div class="grid grid-cols-1 md:grid-cols-12 gap-4">
        ...
        <div class="col-span-2 flex items-end">
          ...
          >
            <ArtSvgIcon icon="ri:information-line" class="text-amber-400 text-base shrink-0" />
    ...
    <div class="flex flex-col lg:flex-row gap-4 md:gap-6 items-stretch">
      ...
      <div class="w-full lg:w-[30%] shrink-0 space-y-4 lg:sticky lg:top-[140px] lg:self-start">
          ...
          <div class="space-y-5">
            ...
            <div>
              <div class="flex items-center gap-2 mb-2.5 px-1">
                ...
                <span class="text-[10px] font-black text-amber-400 uppercase tracking-wider"
                  >Khách hàng</span
                >
              </div>
            ...
            <div>
              <div class="flex items-center gap-2 mb-2.5 px-1">
                ...
                <span class="text-[10px] font-black text-violet-400 uppercase tracking-wider"
                  >Tài chính</span
                >
              </div>
              <div class="flex flex-wrap gap-1.5">
            ...
            <div>
              <div class="flex items-center gap-2 mb-2.5 px-1">
                ...
                <span class="text-[10px] font-black text-orange-400 uppercase tracking-wider"
        ...
        <div v-if="syntaxError" class="bg-red-500/10 border-2 border-red-500/30 rounded-2xl p-5">
          <div class="flex items-start gap-3">
            <ArtSvgIcon icon="ri:error-warning-line" class="text-red-400 text-xl shrink-0 mt-0.5" />
            <div>
              <h4 class="m-0 text-[11px] font-black text-red-400 uppercase">Lỗi cú pháp</h4>
              <p class="m-0 text-[11px] font-bold text-red-300/80 mt-1">{{ syntaxError }}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="flex-1 min-w-0 flex flex-col lg:sticky lg:top-[140px] lg:self-start">
...
<script setup lang="ts">
  ...
  const editorRef = ref<{ setHtml?: (html: string) => void; getHtml?: () => string } | null>(null)
  ...
  const getDefaultTemplateContent = (type: string) =>
    normalizeTemplateHtml(defaultTemplateContents[type] || defaultTemplateContents.Sales)
    ...
    tokenSearch.value
      ? customerTokens.filter(
          (t) =>
            ...
            getTokenLabel.value(t.key).toLowerCase().includes(tokenSearch.value.toLowerCase()),
        )
      : customerTokens,
  )
  ...
  const getTokenGroup = (key: string): string => {
    const allTokens = [...customerTokens, ...vehicleTokens, ...financeTokens, ...supplierTokens]
  ...
  const getEditorRoot = () =>
    document.querySelector('.contract-editor [data-slate-editor]') as HTMLElement | null
