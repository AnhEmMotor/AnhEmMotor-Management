  ...
  <div class="finance-contract-page art-full-height flex flex-col gap-4 pb-5">
      ...
      >
        <template #loanInfo="{ row }">
          <div class="flex flex-col">
            ...
            <span class="finance-strong-text finance-term-text">({{ row.term }})</span>
          </div>
        </template>
        ...
        <template #operation="{ row }">
          <div class="finance-operation-cell">
            <div class="finance-operation-main">
              ...
              <span v-else class="finance-operation-placeholder" aria-hidden="true"></span>
            </div>
    ...
    >
      <div v-if="selectedRow" class="finance-dialog-summary">
        <p class="mb-1"> <strong>Số HĐ:</strong> {{ selectedRow.contractNumber }} </p>
        <p class="mb-1"> <strong>Khách hàng:</strong> {{ selectedRow.customer }} </p>
        <p>
          <strong>Số tiền dự kiến:</strong>
          <span class="font-bold text-green-600">{{ selectedRow.loanAmount }}</span>
        </p>
      </div>
      ...
      <ElForm :model="disbursementForm" label-position="top">
        ...
        <ElFormItem label="Chứng từ đối soát chuyển khoản" required>
          ...
          >
            <div v-if="disbursementProofPreviewUrl" class="finance-upload-preview">
              <img :src="disbursementProofPreviewUrl" :alt="disbursementProofName" />
      ...
      <template #footer>
        <span class="dialog-footer">
          <ElButton :disabled="disbursementSubmitting" @click="disbursementModalVisible = false"
            >Hủy</ElButton
          >
          <ElButton type="success" :loading="disbursementSubmitting" @click="submitDisbursement">
            Xác nhận giải ngân
          </ElButton>
        </span>
      </template>
...
<script setup lang="ts">
    ...
    try {
      ...
      tableData.value = items.map((c: any) => {
        ...
        return {
          ...
          term: c.creditPackage?.termMonths != null ? `${c.creditPackage.termMonths} tháng` : '-',
  ...
  const computeStatistics = (items: any[]) => {
    ...
    statistics.pendingDisbursementCount = items.filter((c) => c.status === 'Pending').length
    ...
    statistics.cavetHeldCount = items.filter(
      (c) => c.cavet?.state === 'FinancialCompanyHolds' || c.cavet?.state === 'StoreHoldsOnBehalf',
    ).length
  }
  ...
  const submitDisbursement = async () => {
    ...
    try {
      ...
      ElMessage.error(error?.response?.data?.message ?? 'Không cập nhật được giải ngân.')
    } finally {
