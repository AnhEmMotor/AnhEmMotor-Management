<template>
  <div class="art-card p-5 h-[27.8rem] mb-5 overflow-hidden max-sm:mb-4">
    ...
    <ElScrollbar style="height: 21.55rem" class="w-full">
      ...
      >
        <template #default>
          <ElTableColumn :label="$t('dashboard.ecommerce.product')" prop="product" width="220px">
          ...
          <ElTableColumn :label="$t('dashboard.ecommerce.sales')" prop="sales" />
          <ElTableColumn :label="$t('dashboard.ecommerce.salesTrend')" width="240">
            <template #default="scope">
...
<script setup lang="ts">
  ...
  const getStockStatus = (stock: number): string => {
    ...
    if (stock < STOCK_THRESHOLD.MEDIUM) return t('dashboard.ecommerce.moderateStock')
