...
const productData = ref([
  { name: 'Nhông Sên Dĩa (SSS 428)', costPrice: 280000, sellPrice: 350000, stock: 45, maxStock: 80, sold: 23, trend: [15, 18, 23], status: 'Còn hàng' },
  { name: 'Dầu Nhớt Motul 7100 (1L)', costPrice: 240000, sellPrice: 350000, stock: 120, maxStock: 200, sold: 87, trend: [60, 75, 87], status: 'Còn hàng' },
  { name: 'Lốp Michelin Pilot Street', costPrice: 720000, sellPrice: 850000, stock: 18, maxStock: 50, sold: 34, trend: [40, 38, 34], status: 'Còn hàng' },
  { name: 'Ắc Quy GS GTZ7V (12V-6Ah)', costPrice: 350000, sellPrice: 450000, stock: 3, maxStock: 30, sold: 15, trend: [12, 14, 15], status: 'Sắp hết' },
  { name: 'Phanh ABS Bosch (Winner X)', costPrice: 1200000, sellPrice: 1500000, stock: 3, maxStock: 20, sold: 7, trend: [5, 6, 7], status: 'Sắp hết' },
  { name: 'Gương Chiếu Hậu (Exciter 155)', costPrice: 85000, sellPrice: 120000, stock: 25, maxStock: 60, sold: 41, trend: [30, 35, 41], status: 'Còn hàng' },
  { name: 'Bugi NGK Iridium', costPrice: 130000, sellPrice: 220000, stock: 0, maxStock: 100, sold: 45, trend: [50, 48, 45], status: 'Hết hàng' },
])
...
const avgTurnover = computed(() => {
  const turnovers = productData.value
    .filter((p) => p.stock > 0)
    .map((p) => p.sold / p.stock)
...
const handleExport = () => {
  console.debug('[ProductReport] Export clicked')
}
  ...
  <div class="p-6 rounded-xl shadow-lg bg-white">
    <div class="flex items-start justify-between mb-6">
      <div>
        ...
        <p class="text-gray-500 text-sm">Phân tích doanh thu, lợi nhuận và hiệu suất từng mã hàng</p>
      </div>
        ...
        <tbody class="text-gray-600 text-sm">
          ...
          >
                ...
                <div class="flex-1">
                  <div class="w-full bg-gray-100 rounded-full h-1.5">
                    <div
                      ...
                      :class="product.stock === 0 ? 'bg-gray-300' : product.stock < 5 ? 'bg-yellow-400' : 'bg-green-400'"
            ...
            <td class="py-3 px-4 whitespace-nowrap text-right">
              <span
                class="font-semibold"
                :class="getMargin(product) >= 30 ? 'text-green-600' : getMargin(product) >= 20 ? 'text-yellow-600' : 'text-red-600'"
              >
                {{ getMargin(product) }}%
              </span>
            </td>
