    ...
    <ElCard class="workflow-card">
      <div class="flex flex-col lg:flex-row justify-between items-center gap-6 py-2">
        <div class="min-w-[200px]">
          ...
          <p class="text-sm text-gray-500">Hệ thống tự động ghi nhận và chốt hoa hồng theo trạng thái đơn hàng.</p>
        </div>
    ...
    >
      <ElForm :model="formData" label-width="130px" class="mt-4" label-position="left">
        ...
        <ElFormItem label="Nhóm nhân viên" required>
          <ElSelect v-model="formData.employeeGroupId" class="w-full" placeholder="Nhân viên Kinh doanh">
            <ElOption label="Nhân viên Kinh doanh" :value="1" />
            <ElOption label="Nhân viên Kỹ thuật" :value="2" />
          </ElSelect>
        </ElFormItem>

        <ElDivider border-style="dashed"><span class="text-gray-400 font-bold text-xs uppercase tracking-widest">Phạm vi áp dụng</span></ElDivider>
        ...
        <div class="grid grid-cols-2 gap-4">
          ...
          <ElFormItem v-if="applyTo === 'product'" label="Sản phẩm" label-width="80px">
            <ElSelect v-model="formData.productId" class="w-full" placeholder="Chọn" filterable remote>
              <ElOption v-for="p in products" :key="p.id" :label="p.name" :value="p.id" />
            </ElSelect>
          </ElFormItem>
        </div>

        <ElDivider border-style="dashed"><span class="text-gray-400 font-bold text-xs uppercase tracking-widest">Mức thưởng</span></ElDivider>
        ...
        <div class="bg-gray-50/50 p-5 rounded-2xl border border-gray-100 mb-4">
          ...
          <div class="mt-4 p-4 bg-white rounded-xl border border-blue-50 flex items-start gap-3 shadow-sm">
            ...
            <div class="text-xs text-gray-600 leading-relaxed">
              ...
              <p v-if="formData.isPercentage">
                Tiền thưởng = <span class="text-red-500 font-bold">Giá bán sản phẩm</span> × <span class="text-red-500 font-bold">{{ formData.commissionAmount }}%</span>
              </p>
              <p v-else>
                Tiền thưởng = <span class="text-red-500 font-bold">{{ formatCurrency(formData.commissionAmount) }}</span> cố định trên mỗi đơn vị sản phẩm.
              </p>
            </div>
          </div>
        </div>
...
const categories = ref([{ id: 1, name: 'Xe máy' }, { id: 2, name: 'Phụ tùng' }])
...
const columns = [
  { label: 'Tên chính sách', prop: 'policyName', slot: 'policyName', useSlot: true, minWidth: 200 },
...
const searchItems = [
  ...
  { key: 'group', label: 'Nhóm nhân viên', type: 'select', props: {
    options: [
