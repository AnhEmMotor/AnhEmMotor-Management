...
const warehouseData = ref([
  { name: 'Honda', totalStock: 245, capacity: 300, lowStock: 12, outOfStock: 3, status: 'Bình thường', value: 1250000000 },
  { name: 'Yamaha', totalStock: 198, capacity: 250, lowStock: 8, outOfStock: 1, status: 'Bình thường', value: 980000000 },
  { name: 'Suzuki', totalStock: 156, capacity: 200, lowStock: 15, outOfStock: 5, status: 'Cảnh báo', value: 650000000 },
  { name: 'Kawasaki', totalStock: 89, capacity: 150, lowStock: 22, outOfStock: 8, status: 'Cảnh báo', value: 420000000 },
])
...
const totalValue = computed(() =>
  warehouseData.value.reduce((sum, item) => sum + item.value, 0),
)
...
const handleExport = () => {
  console.debug('[WarehouseReport] Export clicked')
}

const handleImport = () => {
  console.debug('[WarehouseReport] Nhập kho clicked')
}

const handleViewDetail = (item) => {
  console.debug('[WarehouseReport] View detail:', item.name)
}

const handleRestock = (item) => {
  console.debug('[WarehouseReport] Restock:', item.name)
}
</script>
                ...
                <div class="flex-1">
                  <div class="w-full bg-gray-100 rounded-full h-1.5">
                    <div
                      ...
                      :class="capacityPercent(item) > 70 ? 'bg-red-300' : capacityPercent(item) > 40 ? 'bg-red-400' : 'bg-red-600'"
                      :style="{ width: `${capacityPercent(item)}%` }"
                    ></div>
                  </div>
                </div>
                <span class="text-xs text-gray-400 w-10 text-right">{{ capacityPercent(item) }}%</span>
              </div>
            </td>
