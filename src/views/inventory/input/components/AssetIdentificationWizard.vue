          ...
          <ElTableColumn label="Số Khung (Frame No)" min-width="180">
            <template #default="{ row, $index }">
              <ElInput
                ...
                :class="{
                  'is-error': checkDuplicate(row.frameNo, 'frameNo', $index),
                }"
              />
              <span
          ...
          <ElTableColumn label="Số Máy (Engine No)" min-width="180">
            <template #default="{ row, $index }">
              <ElInput
                ...
                placeholder="Quét mã QR..."
                :class="{
                  'is-error': checkDuplicate(row.engineNo, 'engineNo', $index),
                }"
              />
              <span
        ...
        <div class="grid grid-cols-2 gap-6">
          <div class="p-6 bg-primary/5 rounded-xl border border-primary/10">
            ...
            <div class="text-3xl font-bold text-primary">
              {{ form.units.length }}
              <small class="text-sm font-normal">xe</small>
            </div>
          </div>
          <div class="p-6 bg-success/5 rounded-xl border border-success/10">
            ...
            <div class="text-3xl font-bold text-success">
              ...
              <small class="text-sm font-normal text-gray-400">VNĐ</small>
            </div>
          </div>
        </div>

        <ElDescriptions title="Thông tin chung" :column="2" border>
          <ElDescriptionsItem label="Nhà cung cấp"
            >Honda Việt Nam</ElDescriptionsItem
          >
          <ElDescriptionsItem label="Dòng xe"
            >Honda Vision 2024</ElDescriptionsItem
          >
          <ElDescriptionsItem label="Thời gian khởi tạo"
            >04/05/2026 08:30</ElDescriptionsItem
          >
          <ElDescriptionsItem label="Người thực hiện"
            >Admin - Hệ thống</ElDescriptionsItem
          >
        </ElDescriptions>
      </div>
    </div>
...
const checkDuplicate = (val: string, field: string, index: number) => {
  if (!val) return false;
  return form.units.some(
    (u, i) => i !== index && u[field as keyof typeof u] === val,
  );
};
