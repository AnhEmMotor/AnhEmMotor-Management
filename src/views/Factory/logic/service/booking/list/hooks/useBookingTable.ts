...
export function useBookingTable() {
  ...
  const columns = ref<ColumnOption[]>([
    ...
    { prop: 'bookingType', label: 'Loại Yêu Cầu', minWidth: '140', slot: 'type' },
  ...
  const loadData = async () => {
    ...
    try {
      ...
      statistics.value.pending = data.value.filter((x) => x.status === 'Pending').length
      statistics.value.confirmed = data.value.filter((x) => x.status === 'Confirmed').length
    } catch (error: any) {
  ...
  const submitForm = async () => {
    if (!formData.value.fullName || !formData.value.phoneNumber || !formData.value.preferredDate) {
