  ...
  <div class="p-4">
    <div class="flex items-start justify-between gap-4 flex-wrap">
      ...
      <div class="flex items-center gap-2">
        <ElButton :icon="ArrowLeft" @click="goBack" :disabled="submitting"> Quay lại </ElButton>
      ...
      <ElCard>
        <template #header>
          <div class="flex items-center justify-between gap-3">
            <span class="font-semibold">🛡️ CHI TIẾT HỒ SƠ BẢO HÀNH: {{ claim?.claimNumber }}</span>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
          <div class="text-sm text-slate-600">
            <div class="font-semibold text-slate-800 mb-1">Mã đơn hàng / Mã xe liên kết</div>
            <div>
              <span class="font-medium">Số khung (VIN):</span> {{ claim?.vehicleVin ?? 'N/A' }}
            </div>
            <div class="mt-1">
              <span class="font-medium">Biển số:</span> {{ claim?.vehiclePlate ?? 'N/A' }}
            </div>
          </div>

          <div>
            <div class="font-semibold text-slate-800 mb-1">⏳ Tiến độ pipeline</div>
    ...
    <div class="mt-6 grid grid-cols-1 lg:grid-cols-10 gap-4">
      ...
      <div class="lg:col-span-7">
        <ElCard>
          <template #header>
            <span class="font-semibold">(CỘT TRÁI - 70%): CHI TIẾT NGHIỆP VỤ</span>
          </template>

          <!-- 1. Minh chứng kỹ thuật -->
          <div class="mb-5">
            <div class="font-semibold mb-2">📋 1. MINH CHỨNG SỰ CỐ KỸ THUẬT</div>

            <div class="text-sm">
              <div class="font-medium text-slate-700">Mô tả</div>
              <div class="mt-1 text-slate-900">{{ claim?.issueDescription ?? 'N/A' }}</div>
            </div>

            <div class="mt-4">
              <div class="font-medium text-slate-700">Hình ảnh & Video sự cố</div>
              <div v-if="mediaUrls.length === 0" class="mt-2 text-sm text-slate-500">
                Chưa có media.
              </div>
              <div v-else class="mt-2 grid grid-cols-1 sm:grid-cols-3 gap-3">
                <div v-for="(u, i) in mediaUrls" :key="u + '_' + i" class="border rounded p-2">
                  <video v-if="isVideo(u)" controls class="w-full max-h-40 rounded">
                    <source :src="u" />
                  </video>
                  <img v-else :src="u" class="w-full max-h-40 object-cover rounded" />
                  <div class="mt-2 text-xs text-slate-500 break-all">{{ u }}</div>
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="mb-5">
            <div class="font-semibold mb-2">⚙️ 2. DANH SÁCH LINH KIỆN YÊU CẦU BỒI HOÀN</div>

            <ElTable :data="claim?.parts ?? []" border style="width: 100%">
              <ElTableColumn prop="partName" label="Tên Phụ Tùng" min-width="220" />
              <ElTableColumn prop="partCode" label="Mã Phụ Tùng" min-width="160" />
              <ElTableColumn label="Đơn giá công bố" min-width="140">
                <template #default="scope">
                  {{ formatVndZero(scope.row.unitPrice) }}
                </template>
              </ElTableColumn>
              <ElTableColumn prop="statusText" label="Trạng Thái Thẩm Định" min-width="180" />
            </ElTable>
          </div>

          <!-- 3. Xử lý đổi trả đặc biệt -->
          <div>
            <div class="font-semibold mb-2">❌ 3. XỬ LÝ ĐỔI TRẢ ĐẶC BIỆT (THU HỒI XE LỖI)</div>

            <div class="text-sm text-slate-600 mb-3">
              💡 Chỉ sử dụng khi lỗi thuộc diện cần Thu hồi phương tiện.
            </div>

            <div v-if="claim?.isRecall" class="rounded-lg border border-amber-200 bg-amber-50 p-4">
              <div class="font-semibold mb-2"
                >⚠️ Kích hoạt Thu hồi xe lỗi & Cập nhật số khung kho tổng</div
              >
              <ElButton type="warning" :disabled="submitting" @click="handleRecall">
                Kích hoạt Thu hồi & Đổi xe mới
              </ElButton>
            </div>

            <div v-else class="text-sm text-slate-500"> Không thuộc diện thu hồi. </div>
          </div>
        </ElCard>
      </div>
        ...
        <ElCard>
          ...
          <div class="mb-5">
            ...
            <div class="text-sm text-slate-700">
              <div>
                <span class="font-medium">Chủ xe:</span> {{ claim?.customerName ?? 'N/A' }}
              </div>
              <div class="mt-1">
                <span class="font-medium">SĐT:</span> {{ claim?.customerPhone ?? 'N/A' }}
              </div>
              <div class="mt-1">
                <span class="font-medium">Địa chỉ:</span> {{ claim?.customerAddress ?? 'N/A' }}
              </div>

              <div class="mt-3">
                <div class="font-medium mb-1">Thông tin phương tiện</div>
                <div class="mt-1">
                  <span class="font-medium">Màu:</span> {{ claim?.vehicleColor ?? 'N/A' }}
                </div>
                <div class="mt-1">
                  <span class="font-medium">Đời xe:</span> {{ claim?.vehicleYear ?? 'N/A' }}
                </div>
                <div class="mt-1">
                  <span class="font-medium">Hạn BH:</span> {{ claim?.warrantyRemaining ?? 'N/A' }}
                </div>
              </div>
            </div>
          </div>
...
<script setup lang="ts">
  ...
  const statusTagType = computed(() => {
    ...
    if (s.includes('approved') || s.includes('completed') || s.includes('replaced'))
      return 'success'
  ...
  const pipelineSteps = computed(() => {
    ...
    const steps = [
      ...
      { key: 'approved', label: 'Đã duyệt bồi hoàn', isActive: s.includes('approved') },
      { key: 'replaced', label: 'Thợ thay thế', isActive: s.includes('replaced') },
      { key: 'completed', label: 'Hoàn tất - Đóng hồ sơ', isActive: s.includes('completed') },
    ]
