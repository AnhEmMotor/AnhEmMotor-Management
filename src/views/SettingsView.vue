...
const needsDeposit = computed(() =>
  toggles.deposit && testOrderValue.value > settings.maxOrder,
)
...
<template>
  <div class="p-6 rounded-xl shadow-lg bg-white">
    <div class="flex items-start justify-between mb-6 sticky top-0 bg-white z-10 pb-4 border-b border-gray-200">
      <div>
        <h1 class="text-3xl font-bold mb-1 text-gray-800">Cài Đặt Bán Hàng</h1>
        <p class="text-gray-500 text-sm">Quản lý quy tắc đặt cọc, giới hạn đơn hàng và cảnh báo tồn kho</p>
      </div>
        ...
        <div class="border border-gray-200 rounded-lg overflow-hidden">
          <div class="flex items-center justify-between px-5 py-4 bg-gray-50 border-b border-gray-200">
            ...
            <label class="relative inline-flex items-center cursor-pointer">
              ...
              <div class="w-9 h-5 bg-gray-200 peer-focus:ring-2 peer-focus:ring-red-200 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border after:border-gray-300 after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:bg-red-500"></div>
            </label>
          </div>
          ...
          <div v-if="toggles.deposit" class="p-5 space-y-4">
            <div class="grid grid-cols-2 gap-4">
              <div>
                <label class="block text-xs font-medium text-gray-500 mb-1.5">Đơn hàng vượt quá</label>
                <div class="relative">
                  <input
                    type="text"
                  ...
                  />
                  <span class="absolute right-3 top-1/2 -translate-y-1/2 text-xs text-gray-400 font-medium">VNĐ</span>
                </div>
              </div>
              <div>
            ...
            <div class="p-3 bg-red-50 border border-red-100 rounded-lg">
              ...
              <div class="text-sm font-medium" :class="needsDeposit ? 'text-red-700' : 'text-gray-500'">
                <template v-if="needsDeposit">
                  → Khách phải cọc: <span class="font-bold">{{ formatNumber(depositAmount) }} VNĐ</span>
                </template>
                <template v-else>
                  → Không yêu cầu cọc (đơn hàng chưa vượt ngưỡng)
                </template>
              </div>
            </div>
          </div>
      ...
      <div class="space-y-6">
        <div class="border border-gray-200 rounded-lg overflow-hidden">
          ...
          <div v-if="toggles.orderLimit" class="p-5 space-y-4">
            <div>
              <label class="block text-xs font-medium text-gray-500 mb-1.5">Số lượng xe tối đa / đơn</label>
              <div class="relative w-40">
                ...
                />
                <span class="absolute right-3 top-1/2 -translate-y-1/2 text-xs text-gray-400 font-medium">chiếc</span>
              </div>
            </div>
            <div class="flex items-start gap-2 p-3 bg-yellow-50 border border-yellow-100 rounded-lg">
              <span class="text-yellow-500 text-sm mt-0.5">ℹ️</span>
              <p class="text-xs text-yellow-700">
                Đơn hàng vượt quá <span class="font-bold">{{ settings.maxCount }} xe</span> sẽ chuyển sang trạng thái
                <span class="font-semibold">"Chờ duyệt"</span> thay vì xác nhận ngay. Admin cần phê duyệt thủ công.
              </p>
            </div>
          </div>
        </div>
        ...
        <div class="border border-gray-200 rounded-lg overflow-hidden">
          ...
          <div v-if="toggles.stockWarning" class="p-5 space-y-4">
            <div>
              <label class="block text-xs font-medium text-gray-500 mb-1.5">Ngưỡng cảnh báo chung</label>
              <div class="relative w-40">
                <input
                  type="number"
                ...
                />
                <span class="absolute right-3 top-1/2 -translate-y-1/2 text-xs text-gray-400 font-medium">sản phẩm</span>
              </div>
            </div>
            <div class="p-3 bg-gray-50 border border-gray-200 rounded-lg">
              <p class="text-xs text-gray-500 mb-2">Xem trước hiển thị:</p>
              <div class="flex items-center gap-3">
                <span class="text-sm text-gray-700">Tồn kho: <span class="font-bold">{{ settings.stockLevel }}</span></span>
