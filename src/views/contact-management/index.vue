          ...
          <div class="split-layout">
            <div class="list-panel">
              <div class="list-header">
                <ElInput v-model="searchQuery" :placeholder="$t('contact.searchPlaceholder')" class="flex-1" clearable @input="onSearch">
                  <template #prefix>
                    <ArtSvgIcon icon="ri:search-2-line" />
                  </template>
                </ElInput>
                <ElSelect v-model="statusFilter" :placeholder="$t('contact.allStatus')" clearable style="width: 160px" @change="onFilterChange">
              ...
              >
                <ElTableColumn :label="$t('contact.columnIdCustomer')" min-width="160">
                  <template #default="{ row }">
                    <div class="flex flex-col py-0.5">
                      ...
                      <span class="text-[11px] text-slate-400 font-semibold truncate">{{ row.contact?.fullName || row.email }}</span>
                    </div>
                  </template>
                </ElTableColumn>
                <ElTableColumn prop="subject" :label="$t('contact.columnSubject')" min-width="170" show-overflow-tooltip />
                ...
                <ElTableColumn prop="createdAt" :label="$t('contact.columnCreatedAt')" width="120" />
              </ElTable>
              ...
              <div v-else class="detail-content flex flex-col h-full">
                ...
                <div class="detail-header p-5 border-b border-slate-100 bg-[#FAFCFF]">
                  ...
                  <div class="flex items-center gap-3 mt-1 text-xs text-slate-500">
                    ...
                    <span v-if="activeItemOrderCode" class="font-semibold text-slate-700">Đơn hàng: {{ activeItemOrderCode }}</span>
                  </div>
                </div>
                ...
                <div class="flex-1 overflow-y-auto p-5 space-y-5">
                  <!-- Content Body -->
                  <div>
                    <h4 class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Nội dung yêu cầu</h4>
                    <div class="bg-slate-50 border border-slate-100 rounded-xl p-4 shadow-sm">
                      <p class="text-xs text-slate-600 leading-relaxed m-0 whitespace-pre-line font-medium">{{ activeItemContent }}</p>
                    </div>
                  </div>

                  <!-- Reply -->
                  <div class="pt-2">
                    <h4 class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Soạn thảo phản hồi</h4>
                    <ElInput v-model="replyDraft" type="textarea" :rows="3" :placeholder="$t('contact.replyDialog.contentPlaceholder')" class="reply-input text-xs" resize="none" />
                    <div class="flex justify-between items-center mt-3">
                      <div class="flex gap-2">
                        <ElButton v-if="activeItem && activeItem.status !== 'Closed'" size="small" type="success" plain class="font-bold text-[10px]" @click="handleStatus('Closed')">
                          <ArtSvgIcon icon="ri:checkbox-circle-line" class="mr-1" />Đóng yêu cầu
                        </ElButton>
                        <ElButton v-if="activeItem && activeItem.status !== 'InProgress'" size="small" type="primary" plain class="font-bold text-[10px]" @click="handleStatus('InProgress')">
                          <ArtSvgIcon icon="ri:loader-4-line" class="mr-1" />Đang xử lý
                        </ElButton>
                        <ElButton v-if="activeItem && activeItem.status !== 'New'" size="small" type="info" plain class="font-bold text-[10px]" @click="handleStatus('New')">
                          <ArtSvgIcon icon="ri:arrow-go-back-line" class="mr-1" />Mở lại (Mới)
                        </ElButton>
                        <ElButton size="small" plain class="font-bold text-[10px]" @click="openAssignDialog">
                          <ArtSvgIcon icon="ri:user-settings-line" class="mr-1" />Phân công
                        </ElButton>
                      </div>
                      <ElButton type="primary" class="font-bold text-xs uppercase" style="background: #001529; border-color: #001529;" @click="handleReply">
                ...
                <div class="border-t border-slate-100 p-5 bg-slate-50">
                  <div class="flex items-center justify-between mb-2">
                    <h4 class="text-xs font-bold text-indigo-500 uppercase tracking-wider m-0">Ghi chú nội bộ</h4>
                    <span class="text-[10px] text-slate-400 italic">Chỉ hiển thị với nhân viên điều hành</span>
                  </div>
                  <div class="flex gap-2 items-start">
                    <ElInput v-model="noteDraft" type="textarea" :rows="2" placeholder="Nhập đánh giá nội bộ..." class="flex-1 text-xs" resize="none" />
                    <ElButton type="primary" class="font-bold text-xs" style="background: #4f46e5; border-color: #4f46e5; height: 50px;" @click="saveNote">Lưu</ElButton>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div class="split-layout">
            <div class="list-panel">
              ...
              <ElTable :data="tableData" :loading="contactStore.loading" class="contact-table" :header-cell-style="tableHeaderStyle" @row-click="selectItem">
                ...
                <ElTableColumn prop="customerName" :label="$t('contact.columnCustomer')" min-width="140" show-overflow-tooltip />
                <ElTableColumn prop="feedbackArea" :label="$t('contact.columnCategory')" width="140" show-overflow-tooltip />
                <ElTableColumn :label="$t('contact.columnStatus')" width="120">
                  <template #default="{ row }">
            ...
            <div class="detail-panel">
              ...
              <div v-else class="detail-content flex flex-col h-full">
                ...
                <div class="detail-header p-5 border-b border-slate-100 bg-[#FAFCFF]">
                  ...
                  <h2 class="text-base font-black text-slate-800 mt-2">{{ activeItemCustomerName }}</h2>
                  <div class="flex items-center gap-3 mt-1 text-xs text-slate-500">
                    <span class="text-blue-600 font-bold">{{ activeItemEmail }}</span>
                    <span class="text-slate-300">|</span>
                    <span>{{ activeItemPhone }}</span>
                    <span class="text-slate-300" v-if="activeItemFeedbackArea">|</span>
                    <span v-if="activeItemFeedbackArea" class="font-semibold text-slate-700">Lĩnh vực: {{ activeItemFeedbackArea }}</span>
                    ...
                    <span v-if="(activeItem as any)?.rating" class="flex items-center gap-0.5">
                      <ArtSvgIcon v-for="n in (activeItem as any).rating" :key="n" icon="ri:star-fill" class="text-amber-400 text-xs" />
                    </span>
                  </div>
                </div>
                ...
                <div class="flex-1 overflow-y-auto p-5 space-y-5">
                  <!-- Content Body -->
                  <div>
                    <h4 class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Nội dung đóng góp</h4>
                    <div class="bg-slate-50 border border-slate-100 rounded-xl p-4 shadow-sm">
                      <p class="text-xs text-slate-600 leading-relaxed m-0 whitespace-pre-line font-medium">{{ activeItemContent }}</p>
                    </div>
                  </div>
                  ...
                  <div class="pt-2">
                    ...
                    <div class="flex justify-between items-center mt-3">
                      <div class="flex gap-2">
                        <ElButton v-if="activeItem && activeItem.status !== 'Resolved'" size="small" type="success" plain class="font-bold text-[10px]" @click="handleStatus('Resolved')">
                          <ArtSvgIcon icon="ri:checkbox-circle-line" class="mr-1" />Giải quyết
                        </ElButton>
                        <ElButton v-if="activeItem && activeItem.status !== 'Read'" size="small" type="primary" plain class="font-bold text-[10px]" @click="handleStatus('Read')">
                          <ArtSvgIcon icon="ri:eye-line" class="mr-1" />Đã đọc
                        </ElButton>
                        <ElButton v-if="activeItem && activeItem.status !== 'Pending'" size="small" type="info" plain class="font-bold text-[10px]" @click="handleStatus('Pending')">
        ...
        <ElTabPane :label="$t('contact.tabCandidate')" name="candidate">
          <div class="split-layout">
            <div class="list-panel">
              ...
              <ElTable :data="tableData" :loading="contactStore.loading" class="contact-table" :header-cell-style="tableHeaderStyle" @row-click="selectItem">
                <ElTableColumn prop="fullName" :label="$t('contact.columnCustomer')" min-width="140" show-overflow-tooltip />
                <ElTableColumn prop="appliedPosition" :label="$t('contact.columnPosition')" min-width="140" show-overflow-tooltip />
                <ElTableColumn label="Tệp CV" width="100">
                  <template #default="{ row }">
                    <ElButton v-if="(row as Contact.JobApplication).cvFileUrl" type="primary" link size="small" @click.stop="downloadCvUrl((row as Contact.JobApplication).cvFileUrl)">
              ...
              <div v-else class="detail-content flex flex-col h-full">
                ...
                <div class="detail-header p-5 border-b border-slate-100 bg-[#FAFCFF]">
                  ...
                  <div class="flex items-center gap-3 mt-1 text-xs text-slate-500">
                    ...
                    <span>{{ activeItemPhone }}</span>
                    <span class="text-slate-300">|</span>
                    <span class="font-semibold text-slate-700">{{ activeItemAppliedPosition }}</span>
                  </div>
                </div>
                ...
                <div class="flex-1 overflow-y-auto p-5 space-y-5">
                  <!-- CV PDF Viewer -->
                  <div>
                    <h4 class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Hồ sơ ứng viên (CV)</h4>
                    <div v-if="activeItem?.cvFileUrl" class="border border-slate-200 rounded-xl overflow-hidden h-[360px] bg-slate-50 shadow-inner">
                      <iframe :src="activeItem.cvFileUrl" class="w-full h-full border-none" />
                    </div>
                    <div v-else class="flex flex-col items-center justify-center py-12 bg-slate-50 border border-dashed border-slate-200 rounded-xl text-slate-400">
                      <ArtSvgIcon icon="ri:file-pdf-2-line" class="text-3xl mb-1.5 text-slate-300" />
                  ...
                  <!-- Cover Letter -->
                  <div>
                    <h4 class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Thư giới thiệu</h4>
                    <div class="bg-slate-50 border border-slate-100 rounded-xl p-4 shadow-sm">
                      <p class="text-xs text-slate-600 leading-relaxed m-0 whitespace-pre-line font-medium">{{ activeItemCoverLetter || "Không có thư giới thiệu." }}</p>
                    </div>
                  </div>
                </div>
                ...
                <div class="border-t border-slate-100 p-5 bg-slate-50 space-y-4">
                  <!-- Process Actions -->
                  <div>
                    <h4 class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Quy trình xử lý hồ sơ</h4>
                    <div class="flex gap-2">
                      <ElButton v-if="activeItem && activeItem.status !== 'Interview'" type="primary" class="flex-1 font-bold text-xs uppercase" style="background: #3b82f6; border-color: #3b82f6;" @click="handleStatus('Interview')">
                        <ArtSvgIcon icon="ri:calendar-event-line" class="mr-1" />Hẹn phỏng vấn
                      </ElButton>
                      <ElButton v-if="activeItem && activeItem.status !== 'Offer'" type="success" class="flex-1 font-bold text-xs uppercase" style="background: #10b981; border-color: #10b981;" @click="handleStatus('Offer')">
                        <ArtSvgIcon icon="ri:checkbox-circle-line" class="mr-1" />Nhận việc (Offer)
                      </ElButton>
                      <ElButton v-if="activeItem && activeItem.status !== 'Rejected'" type="danger" class="flex-1 font-bold text-xs uppercase" style="background: #ef4444; border-color: #ef4444;" @click="handleStatus('Rejected')">
                        <ArtSvgIcon icon="ri:close-circle-line" class="mr-1" />Từ chối
                      </ElButton>
                      <ElButton size="small" plain class="font-bold text-[10px] w-28 shrink-0" @click="openAssignDialog">
                        <ArtSvgIcon icon="ri:user-settings-line" class="mr-1" />Phân công
                      </ElButton>
                    </div>
                  </div>
...
import { SupportStatuses, FeedbackStatuses, CandidateStatuses } from '@/infrastructure/api/contact.api'
...
const statusStyle = (status: string) => {
  const m: Record<string, string> = {
    ...
    Read: 'bg-amber-50 text-amber-600 border-amber-100', Closed: 'bg-slate-100 text-slate-600 border-slate-200', Resolved: 'bg-emerald-50 text-emerald-600 border-emerald-100',
...
const downloadCvUrl = (url: string) => { if (url) window.open(url, '_blank') }
