<template>
  <div class="lead-management flex flex-col gap-5 pb-8">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
      ...
      <ArtStatsCard
        title="Đang chăm sóc"
        :count="data.filter(i => i.status !== 'New' && i.status !== 'Lost' && i.status !== 'Won').length"
    ...
    <div class="content-section" v-loading="loading">
      <div v-if="data.length > 0" class="flex flex-col gap-4">
        <div 
          ...
          :class="{ 'is-selected': selectedIds.includes(lead.id), 'is-critical': isCriticalWait(lead) }"
        >
          <!-- Bulk Checkbox -->
          <div class="px-5 flex-cc border-r border-gray-50 h-full min-h-[100px]">
            <ElCheckbox :model-value="selectedIds.includes(lead.id)" @change="toggleSelect(lead.id)" size="large" />
          </div>
          ...
          <div class="flex flex-1 items-center p-5 gap-6">
            ...
            <div class="priority-column flex-shrink-0 relative">
              ...
              <div class="flex flex-col items-center gap-1.5">
                ...
                >
                  ...
                  <span class="text-[10px] tracking-tighter uppercase">{{ getPriority(lead).label }}</span>
                </div>
                <div class="flex items-center gap-1 text-[10px] font-bold" :class="dayjs().diff(dayjs(lead.createdAt), 'hour') >= 24 ? 'text-red-500' : 'text-gray-400'">
            ...
            <div class="identity-column flex-shrink-0 w-72">
              <div class="flex items-center gap-4">
                <ElAvatar :size="54" class="bg-gradient-to-tr from-gray-100 to-gray-200 text-gray-500 font-bold text-xl border-2 border-white">
                  {{ lead.fullName.charAt(0) }}
                </ElAvatar>
                <div class="flex flex-col gap-1">
                  <h4 class="m-0 text-gray-900 font-extrabold text-lg line-clamp-1">{{ lead.fullName }}</h4>
                  <div class="flex items-center gap-2">
                    <span class="text-sm text-gray-500 font-bold">{{ lead.phoneNumber }}</span>
                    <div class="flex gap-1.5 ml-1">
                      <a :href="'tel:' + lead.phoneNumber" class="size-7 bg-green-500 text-white rounded-lg flex-cc hover:bg-green-600 transition-all">
                        <ArtSvgIcon icon="ri:phone-fill" class="text-xs" />
                      </a>
                      <div class="size-7 bg-blue-500 text-white rounded-lg flex-cc hover:bg-blue-600 transition-all">
            ...
            <div class="interest-column flex-1 px-4 border-l border-r border-gray-50">
              <div class="flex flex-col gap-2">
                <div class="flex items-center gap-2">
                  <span class="text-base font-bold text-gray-700">{{ lead.interestedVehicle || 'Chưa xác định' }}</span>
                  <ElTag v-if="lead.interestedVehicle" size="small" :type="getVehicleBrandType(lead.interestedVehicle)" effect="dark" class="rounded-md border-none px-2 uppercase font-bold text-[10px]">
            ...
            <div class="status-column w-44 px-4 flex flex-col items-center justify-center">
              <ElTag :type="getStatusType(lead.status)" class="status-tag-premium w-full" effect="dark">
            ...
            <div class="time-column w-48 text-right pr-4 border-l border-gray-50">
              <div class="flex flex-col gap-1.5 items-end">
                <span class="text-[11px] text-gray-400 font-bold uppercase tracking-tighter">Sale phụ trách</span>
                <ElSelect :model-value="lead.saleId || null" size="small" class="sale-select" :class="{ 'is-unassigned': !lead.saleId }" :placeholder="!lead.saleId ? 'CHƯA BÀN GIAO' : 'Giao Sale...'">
            ...
            <div class="action-column flex-shrink-0">
              <ElButton type="danger" circle class="size-11 flex-cc shadow-lg shadow-red-100 hover:scale-110 transition-transform" @click="$router.push('/customer/profile')">
...
<script setup lang="ts">
  ...
  const searchItems = [
    { key: 'fullName', label: 'Tìm kiếm nhanh', type: 'input', props: { placeholder: 'Họ tên, SĐT...', clearable: true } },
    { key: 'source', label: 'Nguồn khách', type: 'select', props: { placeholder: 'Chọn nguồn...', clearable: true, options: [
      { label: 'Facebook', value: 'Facebook' }, { label: 'Website', value: 'Website' }
    ]}}
  ]
  ...
  const getVehicleBrandType = (v: string) => v.toLowerCase().includes('honda') ? 'danger' : 'primary'
  const getBrand = (v: string) => v.toLowerCase().includes('honda') ? 'Honda' : 'Hãng khác'
  const isNeglected = (lead: any) => dayjs().diff(dayjs(lead.createdAt), 'hour') > 24 && lead.status === 'New'
...
<style lang="scss" scoped>
  .lead-management {
    .search-bar-wrapper {
      :deep(.el-form-item__label) { white-space: nowrap !important; font-weight: 600; }
    }
    .lead-row-card {
      position: relative;
      &.is-critical { animation: critical-blink 2s infinite; }
      .neglected-ribbon {
        position: absolute; top: -8px; right: -8px; background: #ff4d4f; color: white;
        font-size: 10px; font-weight: 800; padding: 4px 12px; border-radius: 6px; z-index: 20;
      }
      .priority-badge {
        width: 100px; height: 60px; border-radius: 12px; display: flex; flex-direction: column;
        align-items: center; justify-content: center; gap: 2px;
        &.blinking { animation: badge-blink 1s infinite; }
      }
      .sale-select.is-unassigned {
        :deep(.el-input__inner::placeholder) { color: #ef4444; font-weight: 800; }
        :deep(.el-input__wrapper) { background-color: #fef2f2; }
      }
    }
  }
  @keyframes critical-blink { 50% { border-color: #ff4d4f; background-color: #fff1f0; } }
  @keyframes badge-blink { 50% { opacity: 0.8; transform: scale(1.05); } }
  .custom-scrollbar {
