<template>
  <div class="purchasing-pipeline-page flex flex-col gap-6 pb-10 h-screen">
    ...
    <div class="grid grid-cols-5 gap-4 px-4 pt-4">
      <div v-for="stat in pipelineStats" :key="stat.label" class="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 flex items-center gap-4">
        ...
        <div class="flex flex-col">
          ...
          <div class="flex items-baseline gap-2">
            ...
            <span v-if="stat.value" class="text-[10px] text-emerald-500 font-bold">{{ stat.value }}</span>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="px-4 flex items-center justify-between">
      <div class="flex gap-3">
        <ElSelect v-model="filterSale" placeholder="Lọc theo Sale" class="w-48 premium-select" clearable>
          <ElOption v-for="sale in salesList" :key="sale.id" :label="sale.name" :value="sale.id" />
        </ElSelect>
        <ElSelect v-model="filterVehicle" placeholder="Dòng xe" class="w-40 premium-select" clearable>
        ...
        >
          ...
          <div class="flex items-center justify-between px-3 py-4">
            <div class="flex items-center gap-2">
              ...
              <h4 class="m-0 text-xs font-black text-gray-700 uppercase tracking-wider">{{ column.title }}</h4>
            </div>
            <ElTag size="small" effect="plain" round class="font-bold">{{ column.items.length }}</ElTag>
          </div>
          ...
          >
            ...
            >
              <!-- Priority Indicator -->
              <div class="absolute top-0 left-4 w-6 h-1 rounded-b-full" :class="deal.priority === 'Urgent' ? 'bg-red-500' : 'bg-orange-400'"></div>

              <div class="flex justify-between items-start mb-3 mt-1">
                <div class="flex flex-col">
                  <span class="text-sm font-black text-gray-800 line-clamp-1">{{ deal.customerName }}</span>
                  <span class="text-[10px] font-bold text-blue-500 uppercase">{{ deal.vehicle }}</span>
                </div>
                <div class="flex gap-1">
                  <div class="size-7 bg-blue-50 text-blue-500 rounded-lg flex-cc hover:bg-blue-500 hover:text-white transition-all cursor-pointer">
                    <ArtSvgIcon icon="ri:chat-1-fill" class="text-xs" />
                  </div>
                  <div class="size-7 bg-green-50 text-green-500 rounded-lg flex-cc hover:bg-green-500 hover:text-white transition-all cursor-pointer">
              ...
              <div class="flex justify-between items-end mt-4 pt-3 border-t border-gray-50">
                <div class="flex items-center gap-1.5">
                  ...
                  <span class="text-[10px] font-bold text-gray-400" :class="{ 'text-red-500': isStale(deal) }">
                    {{ deal.timeInStage }}
                  </span>
                </div>
                <div class="flex -space-x-2">
                  <ElTooltip :content="deal.saleName">
                    <ElAvatar :size="20" class="border border-white bg-gray-200 text-[8px] font-black">
...
<script setup lang="ts">
  ...
  const pipelineStats = ref([
    { label: 'Tổng Deal', count: 42, icon: 'ri:exchange-funds-line', bg: 'bg-blue-600', value: '+12% mtd' },
    { label: 'Đang tư vấn', count: 15, icon: 'ri:customer-service-2-line', bg: 'bg-indigo-500' },
    { label: 'Đã đặt cọc', count: 8, icon: 'ri:money-dollar-circle-line', bg: 'bg-orange-500', value: '420tr' },
    { label: 'Chờ biển số', count: 12, icon: 'ri:government-line', bg: 'bg-purple-500' },
    { label: 'Đã giao xe', count: 7, icon: 'ri:checkbox-circle-line', bg: 'bg-emerald-500', value: 'Goal 85%' }
  ])
  ...
  const boardColumns = ref([
    {
      ...
      color: '#3b82f6',
      items: [
        { id: 101, customerName: 'Nguyễn Hoàng Long', vehicle: 'Winner X 2024', priority: 'High', timeInStage: '2 ngày', saleName: 'Admin' },
        { id: 102, customerName: 'Trần Minh Tâm', vehicle: 'SH 125i', priority: 'Urgent', timeInStage: '5 giờ', saleName: 'Sale A' }
      ]
    },
    {
      id: 'TestDrive',
      ...
      color: '#6366f1',
      items: [
        { id: 103, customerName: 'Lê Văn Tám', vehicle: 'Air Blade 160', priority: 'High', timeInStage: '3 ngày', saleName: 'Sale B' }
      ]
    },
    {
      id: 'Deposit',
      ...
      color: '#f97316',
      items: [
        { id: 104, customerName: 'Phạm Thị Nở', vehicle: 'Vision 2024', priority: 'Urgent', timeInStage: '1 ngày', saleName: 'Admin' }
      ]
    },
    {
      id: 'Processing',
      ...
      color: '#a855f7',
      items: [
        { id: 105, customerName: 'Hoàng Xuân Vinh', vehicle: 'Winner X', priority: 'High', timeInStage: '4 ngày', saleName: 'Sale A' }
      ]
    },
    {
      id: 'Delivered',
      ...
      color: '#10b981',
      items: [
        { id: 106, customerName: 'Bùi Tiến Dũng', vehicle: 'SH Mode', priority: 'High', timeInStage: 'Vừa chốt', saleName: 'Sale B' }
      ]
    }
  ])
